...
### 1. Workspace Cargo.toml (`/home/user/nibleaf-rs/Cargo.toml`)
...
- **nibleaf-api/Cargo.toml**: Changed all external dependencies to use `workspace = true`
- **nibleaf-sites/Cargo.toml**: Changed all external dependencies to use `workspace = true`
- **nibleaf-middleware/Cargo.toml**: Changed all external dependencies to use `workspace = true`
- **nibleaf-auth/Cargo.toml**: Changed to use workspace dependencies for jsonwebtoken and argon2
- **nibleaf-storage/Cargo.toml**: Changed aws-sdk-s3 and aws-config to use workspace = true
- **nibleaf-queue/Cargo.toml**: Changed redis, deadpool-redis, and apalis to use workspace = true
- **nibleaf-search/Cargo.toml**: Changed to use workspace dependencies
- **nibleaf-worker/Cargo.toml**: Changed apalis to use workspace = true
...
#### `/home/user/nibleaf-rs/crates/nibleaf-middleware/src/app_state.rs`
- Updated AuthService instantiation to use `AuthService::new(config.auth.clone(), db.clone())`

### 4. Bug Fixes

Fixed a critical bug in `nibleaf-auth/src/lib.rs`:
...
1. `/home/user/nibleaf-rs/Cargo.toml` - Workspace dependencies upgraded
2. `/home/user/nibleaf-rs/crates/nibleaf-api/Cargo.toml` - Use workspace deps
3. `/home/user/nibleaf-rs/crates/nibleaf-sites/Cargo.toml` - Use workspace deps
4. `/home/user/nibleaf-rs/crates/nibleaf-middleware/Cargo.toml` - Use workspace deps
5. `/home/user/nibleaf-rs/crates/nibleaf-auth/Cargo.toml` - Use workspace deps
6. `/home/user/nibleaf-rs/crates/nibleaf-storage/Cargo.toml` - Use workspace deps
7. `/home/user/nibleaf-rs/crates/nibleaf-queue/Cargo.toml` - Use workspace deps
8. `/home/user/nibleaf-rs/crates/nibleaf-search/Cargo.toml` - Use workspace deps
9. `/home/user/nibleaf-rs/crates/nibleaf-worker/Cargo.toml` - Use workspace deps
10. `/home/user/nibleaf-rs/crates/nibleaf-auth/src/jwt.rs` - jsonwebtoken 9.x compatibility
11. `/home/user/nibleaf-rs/crates/nibleaf-auth/src/session.rs` - jsonwebtoken 9.x compatibility
12. `/home/user/nibleaf-rs/crates/nibleaf-auth/src/lib.rs` - Fixed AuthService constructor
