...
//!
//! The trait is designed so that nibleaf-biz can be tested against a fake
//! implementation without a live storage backend.

use async_trait::async_trait;
use bytes::Bytes;
use nibleaf_config::StorageConfig;
...
pub trait Storage: Send + Sync {
    ...
    async fn upload_target(&self, key: &str, expires_in: Duration) -> Result<UploadTarget, AppError>;
    
    /// Get a download target (presigned URL or server endpoint)
    async fn download_target(&self, key: &str, expires_in: Duration) -> Result<DownloadTarget, AppError>;
...
pub fn create_storage(config: &StorageConfig) -> Result<Box<dyn Storage>, AppError> {
    match config.backend.as_str() {
        "local" => {
            let root_dir = config.local_root
                .as_deref()
                .unwrap_or("./storage");
        ...
        "s3" => {
            ...
            #[cfg(not(feature = "s3"))]
            {
                Err(AppError::Storage("S3 backend requires the 's3' feature to be enabled".to_string()))
            }
        }
        _ => Err(AppError::Storage(format!("Unknown storage backend: {}", config.backend))),
    }
}
...
impl Storage for LocalFsStorage {
    ...
    async fn upload_target(&self, key: &str, _expires_in: Duration) -> Result<UploadTarget, AppError> {
        // For local storage, uploads are server-mediated
        Ok(UploadTarget::ServerMediated(format!("/api/app/assets/{}/upload", key)))
    }
    
    async fn download_target(&self, key: &str, _expires_in: Duration) -> Result<DownloadTarget, AppError> {
        // For local storage, downloads are server-mediated
        Ok(DownloadTarget::ServerMediated(format!("/api/app/assets/{}", key)))
    }
    ...
    async fn exists(&self, key: &str) -> Result<bool, AppError> {
        ...
        Ok(tokio::fs::try_exists(&path).await.map_err(|e| {
            AppError::Storage(format!("Failed to check file existence: {}", e))
        })?)
    }
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<String>, AppError> {
        ...
        if prefix_path.exists() {
            ...
            while let Some(entry) = dir.next_entry().await.map_err(|e| {
                AppError::Storage(format!("Failed to read directory entry: {}", e))
            })? {
                let path = entry.path();
                let key = path.strip_prefix(root_path)
                    .unwrap_or(&path)
                    .to_string_lossy()
                    .replace('\\', "/");
                
                if entry.file_type().await.map_err(|e| {
                    AppError::Storage(format!("Failed to get file type: {}", e))
                })?.is_file() {
                    keys.push(key);
                }
            }
...
impl S3Storage {
    ...
    pub fn new(
        ...
        let config = aws_config::from_env()
            ...
            .load_defaults(aws_sdk_s3::config::Builder::from(&aws_config::load_from_env().await))
            .await;
    ...
    fn get_public_url(&self, key: &str) -> String {
        ...
        if self.use_path_style {
            format!("https://{}/{}/{}", self.client.endpoint_url().host_str().unwrap_or(""), self.bucket, s3_key)
        } else {
            format!("https://{}.{}/{}", self.bucket, self.client.endpoint_url().host_str().unwrap_or(""), s3_key)
        }
    }
}
...
impl Storage for S3Storage {
    ...
    async fn exists(&self, key: &str) -> Result<bool, AppError> {
        ...
        {
            Ok(_) => Ok(true),
            Err(e) if e.to_string().contains("NotFound") || e.to_string().contains("NoSuchKey") => Ok(false),
...
mod tests {
    ...
    async fn test_local_storage_put_get() {
        ...
        storage.put(key, content.clone(), "text/plain").await.unwrap();
    ...
    async fn test_local_storage_list() {
        ...
        storage.put("test/file1.txt", Bytes::from("content1"), "text/plain").await.unwrap();
        storage.put("test/file2.txt", Bytes::from("content2"), "text/plain").await.unwrap();
        storage.put("other/file.txt", Bytes::from("content3"), "text/plain").await.unwrap();
    ...
    async fn test_local_storage_upload_target() {
        ...
        let target = storage.upload_target("test/file.txt", Duration::from_secs(3600)).await.unwrap();
