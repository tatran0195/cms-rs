//! Nibleaf Authentication
//!
//! This crate provides authentication services for Nibleaf, keeping identity
...
use nibleaf_config::AuthConfig;
use nibleaf_db::{PgPool, auth::ApiKeyQueries};
use nibleaf_entity::auth::{ApiKey, ApiKeyResponse, ApiKeyWithSecretResponse, User};
...
impl AuthService {
    ...
    pub fn jwt_validation(&self) -> Validation {
        ...
        validation.set_issuer(&["nibleaf".to_string()]);
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        ...
        let session = nibleaf_db::auth::SessionQueries::get_by_token(&auth_service.pool, &session_cookie)
            .await
        ...
        // Get the user
        let user = nibleaf_db::auth::UserQueries::get_by_id(&auth_service.pool, &session.user_id)
            .await
            .map_err(|_| AppError::InvalidSession)?
            .ok_or(AppError::InvalidSession)?;
