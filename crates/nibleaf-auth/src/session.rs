...
use nibleaf_db::auth::{SessionQueries, UserQueries};
use nibleaf_entity::auth::{Session, SessionResponse, User};
...
impl SessionService {
    ...
    pub async fn create(
        ...
        let expires_at = Utc::now() + Duration::hours(auth_service.config().session_expiration_hours);
        
        // Create the session in the database
        let session = SessionQueries::create(
            pool,
            user_id,
            &session_token,
            expires_at,
        ...
        let token = encode(
            &header,
            &claims,
            auth_service.encoding_key(),
        ).map_err(|e| AppError::Internal(anyhow::anyhow!("Failed to encode session token: {}", e)))?;
