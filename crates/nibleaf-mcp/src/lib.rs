...
use nibleaf_biz::mcp::McpService;
use nibleaf_biz::BizContext;
use nibleaf_entity::mcp::{McpCapabilities, McpRequest, McpResponse, McpToolResult};
...
mod tests {
    ...
    async fn test_get_capabilities() {
        let pool = nibleaf_db::PgPool::connect_lazy(&"postgres://user:pass@localhost/db").unwrap();
        let ctx = BizContext::new(
            pool,
            Arc::new(nibleaf_access_control::NoopAccessControl),
        );
