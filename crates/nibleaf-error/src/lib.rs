...
//! errors in the Nibleaf system, following AppFlowy's pattern of a single
//! error type with one conversion point to HTTP responses.
//!
//! The error variants map directly to Nibleaf's existing closed ErrorCode union:
...
/// The root error type for the Nibleaf application.
/// 
...
mod tests {
    ...
    fn test_error_status_codes() {
        ...
        assert_eq!(AppError::Internal(anyhow::anyhow!("test")).status_code(), StatusCode::INTERNAL_SERVER_ERROR);
    }
    
    #[test]
    fn test_error_codes() {
        assert_eq!(AppError::InvalidApiKey.error_code(), "auth:invalid_api_key");
        assert_eq!(AppError::UsageLimitExceeded("test".to_string()).error_code(), "usage:limit_exceeded");
        assert_eq!(AppError::SearchUnavailable("test".to_string()).error_code(), "search:unavailable");
    }
