...
use nibleaf_config::QueueConfig;
...
pub trait JobQueue: Send + Sync {
    ...
    async fn enqueue_repeatable(&self, job: JobEnvelope, schedule: String) -> Result<JobId, AppError>;
...
pub async fn create_job_queue(config: &QueueConfig) -> Result<Arc<dyn JobQueue>, AppError> {
    match config.backend.as_str() {
        ...
        "redis" => {
            ...
            {
                Err(AppError::Storage("Redis backend requires the 'redis' feature".to_string()))
            }
        }
        _ => Err(AppError::Storage(format!("Unknown queue backend: {}", config.backend))),
    }
}
...
impl JobQueue for MemoryJobQueue {
    ...
    async fn enqueue_repeatable(&self, job: JobEnvelope, _schedule: String) -> Result<JobId, AppError> {
...
impl JobQueue for RedisJobQueue {
    async fn enqueue(&self, job: JobEnvelope) -> Result<JobId, AppError> {
        ...
        redis::cmd("HSET")
            .arg(format!("nibleaf:jobs:{}", job_id.0))
            .arg("metadata")
    ...
    async fn enqueue_repeatable(&self, _job: JobEnvelope, _schedule: String) -> Result<JobId, AppError> {
        // Repeatable jobs would use Redis sorted sets with recurring scores
        Err(AppError::NotFound("Repeatable jobs not yet implemented for Redis backend".to_string()))
    }
    ...
    async fn consume(&self, _consumer_name: &str) -> Result<JobEnvelope, AppError> {
        ...
        let serialized = result.ok_or_else(|| AppError::Storage("No jobs available".to_string()))?;
        ...
        redis::cmd("HSET")
            .arg(format!("nibleaf:jobs:{}", job.id.0))
            .arg("metadata")
            .arg(metadata.to_string())
            .query_async::<_, String>(&mut conn)
            .await?;
    ...
    async fn get_job(&self, job_id: JobId) -> Result<Option<JobEnvelope>, AppError> {
        ...
        if let (Some(metadata), Some(payload)) = (metadata, payload) {
            ...
            Ok(Some(JobEnvelope {
                ...
                payload,
                status: serde_json::from_value(metadata.get("status").cloned().unwrap_or_default())?,
                created_at: chrono::DateTime::parse_from_rfc3339(
                    &metadata.get("created_at").and_then(|v| v.as_str()).unwrap_or("")
                ).unwrap_or(Utc::now()),
                started_at: metadata.get("started_at").and_then(|v| v.as_str())
                    .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok()),
                completed_at: metadata.get("completed_at").and_then(|v| v.as_str())
                    .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok()),
                error_message: metadata.get("error_message").and_then(|v| v.as_str()).map(String::from),
                retry_count: metadata.get("retry_count").and_then(|v| v.as_u64()).unwrap_or(0) as u32,
            }))
        } else {
            Ok(None)
        }
    ...
    async fn list_jobs(
        ...
        Err(AppError::NotFound("List jobs not yet implemented for Redis backend".to_string()))
    }
