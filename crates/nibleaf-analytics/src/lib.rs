...
use nibleaf_config::AnalyticsConfig;
...
pub trait AnalyticsStore: Send + Sync {
    ...
    async fn query_events(
        ...
        limit: Option<i64>,
        offset: Option<i64>,
    ) -> Result<Vec<nibleaf_entity::analytics::AnalyticsEvent>, AppError>;
...
pub async fn create_analytics_store(
    ...
    match config.backend.as_str() {
        ...
        "clickhouse" => {
            ...
            {
                Err(AppError::Storage("ClickHouse backend requires the 'clickhouse' feature".to_string()))
            }
        }
        _ => Err(AppError::Storage(format!("Unknown analytics backend: {}", config.backend))),
    }
}
