...
impl MarkdownRenderer {
    ...
    fn get_allowed_attributes(&self) -> std::collections::HashMap<&'static str, HashSet<&'static str>> {
        ...
        for tag in self.get_allowed_tags() {
            attrs.entry(tag).or_default().extend(global_attrs.iter().copied());
        }
...
<body>
    ...
    <footer>
        <hr>
        <p>Powered by <a href="https://nibleaf.com">Nibleaf</a></p>
    </footer>
</body>
</html>"#,
            description,
    ...
    pub fn highlight_code(&self, code: &str, language: Option<&str>) -> String {
        ...
        match language {
            Some("rust") => format!("<pre class=\"language-rust\"><code>{}</code></pre>", self.escape_html(code)),
            Some("javascript") | Some("js") => format!("<pre class=\"language-javascript\"><code>{}</code></pre>", self.escape_html(code)),
            Some("typescript") | Some("ts") => format!("<pre class=\"language-typescript\"><code>{}</code></pre>", self.escape_html(code)),
            Some("python") | Some("py") => format!("<pre class=\"language-python\"><code>{}</code></pre>", self.escape_html(code)),
            Some("html") => format!("<pre class=\"language-html\"><code>{}</code></pre>", self.escape_html(code)),
            Some("css") => format!("<pre class=\"language-css\"><code>{}</code></pre>", self.escape_html(code)),
            Some("bash") | Some("sh") => format!("<pre class=\"language-bash\"><code>{}</code></pre>", self.escape_html(code)),
            Some("json") => format!("<pre class=\"language-json\"><code>{}</code></pre>", self.escape_html(code)),
            Some("yaml") | Some("yml") => format!("<pre class=\"language-yaml\"><code>{}</code></pre>", self.escape_html(code)),
            Some("markdown") | Some("md") => format!("<pre class=\"language-markdown\"><code>{}</code></pre>", self.escape_html(code)),
...
mod tests {
    ...
    fn test_anchor_generation() {
        ...
        assert_eq!(renderer.generate_anchor("Special!@#$%Chars"), "specialchars");
    }
}
