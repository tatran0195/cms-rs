...
use host_resolution::{HostResolver, HostResolutionResult};
...
async fn serve_project_index(
    ...
    // Get project
    let project = ProjectQueries::get_by_id(
    ...
    // Get project pages
    let pages = PageQueries::get_by_project(
        &state.biz_context.pool,
        &resolution.project_id,
    ).await?;
    
    // Find the root page (index or home)
    let root_page = pages.iter()
...
async fn render_page(
    state: &Arc<SitesAppState>,
    page: &nibleaf_entity::page::Page,
    project: &nibleaf_entity::project::Project,
...
async fn render_project_listing(
    state: &Arc<SitesAppState>,
    project: &nibleaf_entity::project::Project,
    pages: &[nibleaf_entity::page::Page],
) -> Result<String, AppError> {
    let seo_generator = get_seo_generator(state);
...
fn get_security_headers() -> String {
    ...
    for (name, value) in security.get_headers() {
        headers.push_str(&format!("<meta http-equiv=\"{}\" content=\"{}\">\n", name, value));
    }
        ...
        Some(result) => {
            ...
            Ok(format!(
                ...
                if result.is_custom_domain {
                    ...
                    format!("https://{}.nibleaf.app", project.slug)
                }
            ))
        }
        None => {
...
async fn sitemap_xml_handler(
    ...
    match host_resolver.resolve(&headers).await? {
        Some(result) => {
            ...
            let published_pages: Vec<_> = pages
                .into_iter()
...
async fn security_txt_handler() -> String {
    "Contact: security@nibleaf.com\nEncryption: https://nibleaf.com/.well-known/pgp-key.txt\nAcknowledgments: https://nibleaf.com/security/acknowledgments\nPolicy: https://nibleaf.com/security/policy\nHiring: https://nibleaf.com/jobs".to_string()
}
    ...
    "icons": [
        ...
        {{
            ...
            "type": "image/png"
                ...
                project.name,
                project.name,
                project.description.as_deref().unwrap_or("Documentation powered by Nibleaf")
            ))
        }
        None => {
            // Default manifest
