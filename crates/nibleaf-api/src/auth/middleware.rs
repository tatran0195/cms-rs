...
use nibleaf_entity::auth::{UserResponse, ApiKey};
...
pub struct RequireRoleMiddleware {
    pub role: nibleaf_entity::common::MemberRole,
}

impl RequireRoleMiddleware {
    pub fn new(role: nibleaf_entity::common::MemberRole) -> Self {
        Self { role }
    }
}
...
pub async fn require_org_role(
    user_id: &str,
    org_id: &str,
    required_role: nibleaf_entity::common::MemberRole,
    state: &Arc<AppState>,
) -> Result<(), AppError> {
    match required_role {
        nibleaf_entity::common::MemberRole::Owner => state.biz_context.access_control.require_org_owner(user_id, org_id).await,
        nibleaf_entity::common::MemberRole::Admin => state.biz_context.access_control.require_org_admin(user_id, org_id).await,
...
pub async fn require_project_owner(
    ...
    state.biz_context.access_control.require_project_role(user_id, project_id, nibleaf_entity::common::MemberRole::Owner).await
}
