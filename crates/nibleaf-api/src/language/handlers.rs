...
use utoipa::ToSchema;
use nibleaf_biz::language::LanguageService;
use nibleaf_entity::language::{CreateLanguageRequest, UpdateLanguageRequest, LanguageResponse, ListLanguagesQuery};
...
pub async fn set_default_language_handler(
    ...
    LanguageService::set_default_language(
        &state.biz_context,
        &auth.user.id,
        nibleaf_entity::language::SetDefaultLanguageRequest { language_id },
    ).await?;
