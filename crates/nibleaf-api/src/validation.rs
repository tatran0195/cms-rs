...
use axum::{
    extract::{FromRequest, FromRequestParts, Request, Query, Path},
    http::request::Parts,
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        let Query(value) = Query::<T>::from_request_parts(parts, state).await.map_err(|e| {
            AppError::BadRequest(format!("Invalid query parameters: {}", e))
        })?;
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        let Path(value) = Path::<T>::from_request_parts(parts, state).await.map_err(|e| {
            AppError::BadRequest(format!("Invalid path parameters: {}", e))
        })?;
