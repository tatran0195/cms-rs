...
use cms_entity::{
    ...
    language::{
        CreateLanguageRequest, Language, LanguageResponse, ListLanguagesQuery,
        ListLanguagesResponse, ProjectTranslation, ProjectTranslationResponse,
        SetDefaultLanguageRequest, UpdateLanguageRequest,
    },
};
use uuid::Uuid;
...
impl LanguageService {
    ...
    pub async fn create_language(
        ...
        let existing =
            LanguageQueries::get_by_code(&ctx.pool, &request.project_id, &request.code).await?;
    ...
    pub async fn delete_language(
        ...
        let translation_count =
            ProjectTranslationQueries::count_by_language(&ctx.pool, language_id).await?;

        if translation_count > 0 {
            return Err(AppError::Conflict(
                "Cannot delete a language with translations. Delete or reassign translations \
                 first."
                    .to_string(),
            ));
        }
