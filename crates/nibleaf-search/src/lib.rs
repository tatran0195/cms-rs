...
use nibleaf_config::SearchConfig;
use nibleaf_db::PgPool;
use nibleaf_entity::page::Page;
use nibleaf_entity::search::{SearchHit, SearchOptions, RagAnswer};
...
pub async fn create_search_engine(config: &SearchConfig) -> Result<Arc<dyn SearchEngine>, AppError> {
    match config.backend.as_str() {
        "pgvector" => {
            let engine = PgVectorSearchEngine::new(
                config.pgvector_url.clone().unwrap_or_default(),
            ).await?;
            Ok(Arc::new(engine))
        }
        "qdrant" => {
            ...
            {
                Err(AppError::SearchUnavailable("Qdrant backend requires the 'qdrant' feature".to_string()))
            }
        }
        _ => Err(AppError::SearchUnavailable(format!("Unknown search backend: {}", config.backend))),
    }
}
...
impl PgVectorSearchEngine {
    pub async fn new(database_url: String) -> Result<Self, AppError> {
        let pool = nibleaf_db::create_pool(&database_url).await?;
...
impl SearchEngine for PgVectorSearchEngine {
    async fn hybrid_query(
        ...
        // Normalize tokens
        let _normalized_tokens: Vec<String> = tokens
            .iter()
...
impl QdrantSearchEngine {
    pub async fn new(
        ...
        let mut config = qdrant_client::QdrantClientConfig::from_url(&format!("http://{}:{}", host, port));
