...
use nibleaf_db::notification::NotificationQueries;
use nibleaf_entity::notification::{Notification, NotificationResponse, NotificationType, NotificationStatus};
...
impl NotificationService {
    ...
    pub async fn get_notification_count(
        ctx: &BizContext,
        user_id: &str,
    ) -> Result<nibleaf_entity::notification::NotificationCountResponse, AppError> {
        let unread = Self::get_unread_count(ctx, user_id).await? as i64;
        Ok(nibleaf_entity::notification::NotificationCountResponse {
            total: unread,
            unread,
