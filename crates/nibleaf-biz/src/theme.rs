...
use nibleaf_db::theme::ThemeQueries;
use nibleaf_db::project::ProjectQueries;
use nibleaf_entity::theme::{Theme, ThemeResponse, CreateThemeRequest, UpdateThemeRequest};
...
impl ThemeService {
    ...
    pub async fn get_theme_css(
        ...
        user_id: &str,
        theme_id: &str,
    ) -> Result<nibleaf_entity::theme::ThemeCssVariables, AppError> {
        let theme = ThemeQueries::get_by_id(&ctx.pool, theme_id)
            .await?
            .ok_or_else(|| AppError::NotFound("Theme not found".to_string()))?;
