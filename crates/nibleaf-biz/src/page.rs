...
use nibleaf_db::branch::BranchQueries;
use nibleaf_entity::page::{
    Page, PageResponse, PageListItem, PageTreeNode, CreatePageRequest, UpdatePageRequest,
    ReorderPagesRequest, ListPagesQuery, ListPagesResponse, GetPageTreeResponse,
};
...
impl PageService {
    ...
    pub async fn create_page(
        ...
        ctx.access_control.require_project_role(user_id, project_id, nibleaf_entity::common::MemberRole::Member).await?;
        ...
        if let Some(parent_id) = &request.parent_id {
            if !parent_id.is_empty() {
                ...
                if PageQueries::would_create_cycle(&ctx.pool, parent_id, Some(parent_id)).await? {
                    return Err(AppError::Conflict("Cannot create page: would create a cycle in the page tree".to_string()));
                }
            }
        }
        
        // Check if slug is available
        if !PageQueries::is_slug_available(
            &ctx.pool,
            project_id,
            branch_id,
            &request.slug,
            ...
            return Err(AppError::Conflict("Page with this slug already exists in this branch".to_string()));
        }
    ...
    pub async fn get_page(
        ...
        ctx.access_control.require_project_role(
            user_id,
            &page.project_id,
            nibleaf_entity::common::MemberRole::Viewer,
        ).await?;
    ...
    pub async fn list_pages(
        ...
        Ok(PaginatedResponse::new(
            pages.into_iter().map(|p| p.into()).collect(),
            total as u64,
            page,
            page_size,
        ))
    ...
    pub async fn update_page(
        ...
        ctx.access_control.require_project_role(
            user_id,
            &page.project_id,
            nibleaf_entity::common::MemberRole::Editor,
        ).await?;
        ...
        if let Some(new_parent_id) = &request.parent_id {
            if new_parent_id != page.parent_id.as_deref().unwrap_or("") {
                if PageQueries::would_create_cycle(&ctx.pool, page_id, Some(new_parent_id)).await? {
                    return Err(AppError::Conflict("Cannot move page: would create a cycle in the page tree".to_string()));
                }
            }
        }
    ...
    pub async fn reorder_pages(
        ...
        for page in &pages {
            if page.project_id != project_id || page.branch_id != branch_id {
                return Err(AppError::Conflict("All pages must belong to the same project and branch".to_string()));
            }
        }
    ...
    async fn recompute_path_for_page(
        pool: &PgPool,
        start_page_id: &str,
    ) -> Result<u64, AppError> {
        use nibleaf_db::page::PageQueries;
        ...
        while let Some(page_id) = queue.pop() {
            ...
            // Process children
            let children = PageQueries::get_by_parent(pool, &page.project_id, &page.branch_id, &page_id).await?;
    ...
    pub async fn get_public_page(
        ...
        let page = PageQueries::get_by_id(&ctx.pool, page_path).await?
            .ok_or_else(|| AppError::NotFound("Page not found".to_string()))?;
        Ok(page.into())
    }
