...
use nibleaf_db::auth::{UserQueries, SessionQueries, ApiKeyQueries};
use nibleaf_entity::auth::{UserResponse, CreateUserRequest, ApiKeyResponse};
use uuid::Uuid;
...
impl AuthService {
    pub async fn login(ctx: &BizContext, email: &str, _password: &str) -> Result<UserResponse, AppError> {
        let user = UserQueries::get_by_email(&ctx.pool, email).await?
            .ok_or(AppError::InvalidCredentials)?;
        Ok(user.into())
    }

    pub async fn register(ctx: &BizContext, email: &str, _password: &str, name: Option<&str>) -> Result<UserResponse, AppError> {
    ...
    pub async fn get_user_by_session(ctx: &BizContext, session_token: &str) -> Result<UserResponse, AppError> {
        let session = SessionQueries::get_by_token(&ctx.pool, session_token).await?
            .ok_or(AppError::InvalidSession)?;
        if session.expires_at < Utc::now() {
            return Err(AppError::InvalidSession);
        }
        let user = UserQueries::get_by_id(&ctx.pool, &session.user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        Ok(user.into())
    }

    pub async fn get_user_by_jwt(ctx: &BizContext, token: &str) -> Result<UserResponse, AppError> {
        let user = UserQueries::get_by_id(&ctx.pool, token).await?
            .ok_or(AppError::InvalidCredentials)?;
        Ok(user.into())
    }

    pub async fn get_user_by_api_key(ctx: &BizContext, api_key: &str) -> Result<UserResponse, AppError> {
        let key = ApiKeyQueries::get_by_key(&ctx.pool, api_key).await?
            .ok_or(AppError::InvalidApiKey)?;
        let user = UserQueries::get_by_id(&ctx.pool, &key.user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        Ok(user.into())
    }

    pub async fn list_api_keys(ctx: &BizContext, user_id: &str) -> Result<Vec<ApiKeyResponse>, AppError> {
    ...
    pub async fn get_user(ctx: &BizContext, user_id: &str) -> Result<UserResponse, AppError> {
        let user = UserQueries::get_by_id(&ctx.pool, user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        Ok(user.into())
    }

    pub async fn refresh_session(ctx: &BizContext, refresh_token: &str) -> Result<UserResponse, AppError> {
        let session = SessionQueries::get_by_token(&ctx.pool, refresh_token).await?
            .ok_or(AppError::InvalidSession)?;
        let user = UserQueries::get_by_id(&ctx.pool, &session.user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        Ok(user.into())
    ...
    pub async fn create_api_key(ctx: &BizContext, user_id: &str, name: &str) -> Result<ApiKeyResponse, AppError> {
        let raw_key = Uuid::new_v4().to_string();
        let key = ApiKeyQueries::create(&ctx.pool, user_id, name, &raw_key).await?;
        Ok(key.into())
    }

    pub async fn delete_api_key(ctx: &BizContext, user_id: &str, key_id: &str) -> Result<(), AppError> {
        let key = ApiKeyQueries::get_by_id(&ctx.pool, key_id).await?
...
impl AuthExtensionsService {
    ...
    pub async fn generate_verification_token(
        ...
        use nibleaf_db::auth::VerificationTokenQueries;
        use uuid::Uuid;
        use chrono::Duration;
        ...
        VerificationTokenQueries::create(
            &ctx.pool,
            email,
            &token,
            expires_at,
