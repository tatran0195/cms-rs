...
use nibleaf_db::comment::CommentQueries;
use nibleaf_db::page::PageQueries;
use nibleaf_entity::comment::{Comment, CommentResponse, CreateCommentRequest, UpdateCommentRequest};
...
impl CommentService {
    ...
    pub async fn list_comments(
        ...
        let total = CommentQueries::count_by_page(
            &ctx.pool,
            page_id,
            parent_id,
            resolved,
    ...
    pub async fn get_comment_with_replies(
        ...
        user_id: &str,
        comment_id: &str,
    ) -> Result<nibleaf_entity::comment::CommentWithReplies, AppError> {
        let comment = Self::get_comment(ctx, user_id, comment_id).await?;
        let replies = CommentQueries::get_by_page(&ctx.pool, &comment.page_id, Some(comment_id), None, None, None).await?;
        Ok(nibleaf_entity::comment::CommentWithReplies {
            comment,
