...
use nibleaf_entity::search::{SearchRequest, SearchResponse, SearchResultItem, IndexPageRequest, SearchOptions};
...
impl SearchService {
    ...
    pub async fn search(
        ...
        // Perform the search
        let hits = search_engine.hybrid_query(
            project_id,
            &request.query,
            opts,
        ).await?;
        ...
        let results: Vec<SearchResultItem> = hits.into_iter().map(|h| SearchResultItem {
            page_id: h.page_id,
    ...
    pub async fn get_rag_answer(
        ...
        project_id: &str,
        question: &str,
    ) -> Result<nibleaf_entity::search::RagAnswer, AppError> {
        // Verify project exists
        let _project = ProjectQueries::get_by_id(&ctx.pool, project_id)
            .await?
            .ok_or_else(|| AppError::NotFound("Project not found".to_string()))?;
    ...
    pub async fn list_index_runs(
        _ctx: &BizContext,
        _user_id: &str,
        _query: nibleaf_entity::search::ListSearchIndexRunsQuery,
    ) -> Result<nibleaf_entity::common::PaginatedResponse<nibleaf_entity::search::SearchIndexRunResponse>, AppError> {
        Ok(nibleaf_entity::common::PaginatedResponse::new(vec![], 0, 1, 20))
    }
    ...
    pub async fn get_index_run(
        ...
        _user_id: &str,
        run_id: &str,
    ) -> Result<nibleaf_entity::search::SearchIndexRunResponse, AppError> {
        Err(AppError::NotFound(format!("Search index run not found: {}", run_id)))
    }
    ...
    pub async fn reindex(
        _ctx: &BizContext,
        _user_id: &str,
        _request: nibleaf_entity::search::ReindexRequest,
    ) -> Result<(), AppError> {
        Ok(())
    }
}
...
pub async fn process_search_job(
    ...
    let _project_id = payload.get("project_id").and_then(|v| v.as_str())
    ...
    match job_type {
        ...
        _ => return Err(AppError::InvalidInput("Unknown search job type".to_string())),
    }
