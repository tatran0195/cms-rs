...
use nibleaf_entity::org::{Organization, OrganizationResponse, Member, MemberResponse};
...
struct InvitationRow {
    ...
    organization_id: String,
    email: String,
    role: nibleaf_entity::common::MemberRole,
    token: String,
...
impl From<InvitationRow> for nibleaf_entity::org::Invitation {
...
impl InvitationQueries {
    ...
    pub async fn create(
        ...
        token: &str,
        expires_at: chrono::DateTime<Utc>,
    ) -> Result<nibleaf_entity::org::Invitation, AppError> {
        let id = Uuid::new_v4().to_string();
        let now = Utc::now();
    ...
    pub async fn get_by_token(
        pool: &PgPool,
        token: &str,
    ) -> Result<Option<nibleaf_entity::org::Invitation>, AppError> {
        let row = sqlx::query_as::<_, InvitationRow>(
    ...
    pub async fn list_by_org(
        pool: &PgPool,
        org_id: &str,
    ) -> Result<Vec<nibleaf_entity::org::InvitationResponse>, AppError> {
        ...
        Ok(rows.into_iter()
            .map(|r| nibleaf_entity::org::Invitation::from(r).into())
            .collect())
    }
