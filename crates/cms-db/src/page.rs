...
impl PageQueries {
    ...
    pub async fn update(
        ...
        let current_page: Option<PageRow> =
            sqlx::query_as::<_, PageRow>("SELECT * FROM \"Page\" WHERE id = $1")
                .bind(page_id)
                .fetch_optional(pool)
                .await
                .map_err(|e| AppError::Database(e.into()))?;
        ...
        let mut query_builder: QueryBuilder<Postgres> = QueryBuilder::new("UPDATE \"Page\" SET ");
