//! Nibleaf Access Control
//!
//! This crate provides trait-based access control following AppFlowy's pattern,
//! but with Nibleaf's actual enumerable rules instead of Casbin.
//!
//! Nibleaf's authorization rules are:
...
use nibleaf_entity::common::MemberRole;
...
pub trait AccessControl: Send + Sync {
    ...
    async fn require_project_access(&self, user_id: &str, project_id: &str) -> Result<(), AppError> {
        self.require_project_role(user_id, project_id, MemberRole::Guest).await
    }

    /// Require that the user is a member of the project (Member level or above)
    async fn require_project_member(&self, user_id: &str, project_id: &str) -> Result<(), AppError> {
        self.require_project_role(user_id, project_id, MemberRole::Member).await
    }
}
...
impl ProductionAccessControl {
    ...
    async fn get_org_role(&self, user_id: &str, org_id: &str) -> Result<Option<MemberRole>, AppError> {
        use nibleaf_db::org::MemberQueries;
    ...
    async fn get_project_role(&self, user_id: &str, project_id: &str) -> Result<Option<MemberRole>, AppError> {
        // This would query the project membership
        // For now, we'll use organization membership as a proxy
        // In practice, Nibleaf has project-specific roles
    ...
    async fn has_audience_grant(&self, reader_id: &str, project_id: &str) -> Result<bool, AppError> {
        use nibleaf_db::reader_access::ReaderAudienceQueries;
        
        let has_grant = ReaderAudienceQueries::has_grant_for_project(&self.pool, reader_id, project_id)
            .await?;
...
impl AccessControl for ProductionAccessControl {
    async fn require_org_member(&self, user_id: &str, org_id: &str) -> Result<(), AppError> {
        ...
        if role.is_none() {
            return Err(AppError::AccessDenied("User is not a member of this organization".to_string()));
        }
    ...
    async fn require_branch_grant(
        ...
        use nibleaf_db::reader_access::AudienceGrantQueries;
        
        let has_grant = AudienceGrantQueries::has_grant_for_branch(&self.pool, reader_id, project_id, branch_id)
            .await?;
        
        if !has_grant {
            return Err(AppError::AccessDenied("Reader does not have access to this branch".to_string()));
        }
    ...
    async fn require_org_owner(&self, user_id: &str, org_id: &str) -> Result<(), AppError> {
        ...
        if role != Some(MemberRole::Owner) {
            return Err(AppError::InsufficientRole("User must be the organization owner".to_string()));
        }
    ...
    async fn require_org_admin(&self, user_id: &str, org_id: &str) -> Result<(), AppError> {
        ...
        match role {
            ...
            _ => Err(AppError::InsufficientRole("User must be an organization admin".to_string())),
        }
    }
}
...
mod tests {
    ...
    fn test_noop_access_control() {
        ...
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            ac.require_project_role("user-1", "proj-1", MemberRole::Admin).await.unwrap();
            ac.require_audience_grant("reader-1", "proj-1").await.unwrap();
