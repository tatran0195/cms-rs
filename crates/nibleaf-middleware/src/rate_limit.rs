...
//! which is appropriate for the Nibleaf deployment model (single AWS Windows machine).
...
use governor::{Quota, RateLimiter as GovernorRateLimiter, state::NotKeyed, clock};
use std::{
    collections::HashMap,
...
impl RateLimitClient {
    ...
    pub fn from_request(parts: &axum::http::request::Parts) -> Self {
        ...
        if let Some(api_key) = parts.headers.get(header::HeaderName::from_static("x-api-key")) {
...
struct ClientState {
    ...
    limiter: Arc<GovernorRateLimiter<NotKeyed, governor::state::InMemoryState, clock::DefaultClock>>,
...
pub fn rate_limit_exceeded_response(retry_after: Duration) -> Response {
    ...
    (
        StatusCode::TOO_MANY_REQUESTS,
        [(header::RETRY_AFTER, HeaderValue::from_str(&retry_seconds.to_string()).unwrap_or_else(|_| HeaderValue::from_static("60")))],
...
impl RateLimitMiddleware {
    pub fn new(config: RateLimitConfig) -> Result<Self, String> {
        ...
        Ok(Self { config, limiter: Arc::new(limiter) })
    }
