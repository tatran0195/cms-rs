...
use axum::{
    ...
    http::{Request, StatusCode, HeaderMap, HeaderValue, request::Parts},
    response::Response,
};
use std::time::{Instant, Duration};
use tower::Service;
use tracing::{info, error, warn, debug, Span, Instrument};
...
pub fn record_duration(method: &str, duration: Duration) {
    histogram!("http_request_duration_seconds", "method" => method.to_string()).record(duration.as_secs_f64());
}
...
pub fn init_tracing() {
    tracing_subscriber::fmt()
        .with_env_filter("nibleaf=debug,tower_http=debug,axum=debug")
        .with_target(true)
        .with_line_number(true)
        .init();
}
...
pub async fn start_prometheus_exporter(addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        let server = hyper::Server::bind(&addr)
            .serve(builder.with_http_listener().unwrap());
...
pub async fn start_prometheus_exporter(_addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    warn!("Prometheus exporter requested but prometheus feature is not enabled. Add `prometheus` feature to nibleaf-middleware in Cargo.toml");
    Ok(())
}
...
mod tests {
    ...
    fn test_ensure_request_id() {
        ...
        assert!(!id1.is_empty());
        assert_eq!(headers.get(REQUEST_ID_HEADER).unwrap().to_str().unwrap(), id1);
        
        // With existing ID - should not change
        let existing_id = "existing-123";
        headers.insert(
            REQUEST_ID_HEADER,
            HeaderValue::from_static(existing_id),
        );
        let id2 = ensure_request_id(&mut headers);
        assert_eq!(id2, existing_id);
        assert_eq!(headers.get(REQUEST_ID_HEADER).unwrap().to_str().unwrap(), existing_id);
    }
