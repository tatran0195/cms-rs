...
impl Default for AdminOriginConfig {
    fn default() -> Self {
        ...
        origins.insert("https://admin.nibleaf.com".to_string());
        origins.insert("https://app.nibleaf.com".to_string());
        origins.insert("https://nibleaf.com".to_string());
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for origin in &self.allowed_origins {
            ...
            if !origin.starts_with("http://") && !origin.starts_with("https://") {
                return Err(format!("Invalid origin format: {}. Must start with http:// or https://", origin));
            }
            
            // Must not contain path or query
            if origin.contains('/') || origin.contains('?') || origin.contains('#') {
                return Err(format!("Invalid origin: {}. Must not contain path, query, or fragment", origin));
            }
        }
...
pub fn normalize_origin(origin: &str) -> Result<String, String> {
    ...
    let (scheme, host_port) = match origin_trimmed.split_once("://") {
        ...
        None => return Err(format!("Invalid origin: missing scheme: {}", origin_trimmed)),
    };
...
pub async fn validate_admin_origin(
    ...
    let is_allowed = final_origin.as_ref()
...
pub fn extract_origin_from_request(parts: &Parts) -> Option<String> {
    parts.headers.get(header::ORIGIN)
        .and_then(|h| h.to_str().ok())
