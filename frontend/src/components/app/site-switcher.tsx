  ...
  return (
    <DropdownMenu>
        ...
        render={
          ...
          >
            <span className="grid size-8 shrink-0 place-items-center rounded-lg border border-sidebar-border bg-sidebar-primary/10">
              {selected ? (
                <CMSMark className="size-5.5" />
              ) : (
                <LayoutGrid className="size-4.5 text-sidebar-primary" />
              )}
            </span>
            <span className="min-w-0 flex-1">
              ...
              <span className="block truncate text-muted-foreground text-xs">
                {selected ? t("nav.site") : t("nav.workspace")}
              </span>
            </span>
      ...
      <DropdownMenuContent align="start" className="w-64">
        ...
        <DropdownMenuItem
          onClick={() =>
            navigate({ to: "/app/sites", search: { newSite: true } })
          }
        >
