...
export function SiteSearch({
  ...
  const answer = answerMutation.isPending
    ? null
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-h-[min(80vh,720px)] overflow-hidden p-0 sm:max-w-2xl"
          ...
          ((filtersEnabled && languages.length > 1) ||
            ...
            <div className="flex flex-col gap-2 border-b px-3 py-2.5 sm:flex-row">
              {filtersEnabled && languages.length > 1 ? (
                ...
                >
                  <SelectTrigger
                    aria-label={t("searchFilterLanguage")}
                    className="h-9 min-w-0 flex-1 sm:max-w-56"
                  >
              ...
              {versionFilterEnabled && versions.length > 1 ? (
                <Select
                  onValueChange={(value) =>
                    setSelectedVersion(
                      !value || value === "__default" ? undefined : value,
                    )
                  }
                  value={selectedVersion ?? "__default"}
                >
                  <SelectTrigger
                    aria-label={t("searchFilterVersion")}
                    className="h-9 min-w-0 flex-1 sm:max-w-56"
                  >
                  ...
                  <SelectContent>
                    {versions.map((item) => (
                      <SelectItem
                        key={item.id}
                        value={item.isDefault ? "__default" : item.slug}
                      >
                        {item.name}
                      </SelectItem>
                    ))}
          ...
          {mode === "search" ? (
                ...
                {hits.map((hit, index) => (
                  <CommandItem
                    ...
                    onSelect={() => {
                      ...
                      onOpenChange(false);
                      window.location.href = siteHref(projectId, hit.path, {
            ...
            >
                    ...
                    <div className="space-y-2 border-t pt-3">
                      ...
                      {answer.citations.map((citation) => (
                        <a
                          ...
                          href={siteHref(projectId, citation.path, {
                            lang: selectedLanguage,
                            version: selectedVersion,
                          })}
              ...
              <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4">
                ...
                <Button
                  disabled={query.trim().length < 2 || answerMutation.isPending}
                  onClick={ask}
                  size="sm"
                  type="button"
                >
