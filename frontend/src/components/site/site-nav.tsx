...
const containsPath = (node: NavNode, path: string): boolean =>
  node.kind === "PAGE"
    ? node.path === path
    : node.children.some((child) => containsPath(child, path));
...
export const isVersionIndependentNavNode = (
  node: Pick<NavNode, "id">,
): boolean => node.id.startsWith("openapi:");
...
function NavLink({
  ...
  depth,
  rail,
}: NavItemProps & { node: NavNode; rail?: boolean }) {
  const active = currentPath === node.path;
  return (
    <li>
      <a
        href={siteHref(projectId, node.path, {
          lang,
          version: isVersionIndependentNavNode(node) ? undefined : version,
        })}
        ...
        className={cn(
          ...
          rail &&
            active &&
            "before:absolute before:-start-[13px] before:inset-y-1 before:w-[1.5px] before:rounded-full before:bg-primary",
        )}
      >
        <PageIcon
          name={node.icon}
...
function NavSubGroup({
  ...
  const [open, setOpen] = useState(
    () => !node.id.startsWith("category:") || containsPath(node, currentPath),
  );
  useEffect(() => {
...
function NavItems({
  ...
  rail,
  ...rest
}: NavItemProps & { nodes: NavNode[]; rail?: boolean }) {
  return (
    <ul
      className={cn(
        "space-y-px",
        rail && "ms-3 border-border/70 border-s ps-3",
      )}
    >
      {nodes.map((node) => {
        if (node.kind === "GROUP") {
          return rest.depth === 0 ? (
            <NavSection key={node.id} node={node} {...rest} />
          ) : (
            <NavSubGroup key={node.id} node={node} {...rest} />
          );
        }
