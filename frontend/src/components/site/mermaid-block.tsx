...
export function MermaidBlock({ children }: { children?: React.ReactNode }) {
  ...
  if (failed || !svg) {
    return (
      <pre className="my-5 overflow-x-auto rounded-xl border border-border bg-[#0d1117] p-4 text-sm text-white/80 [direction:ltr]">
        {chart}
      </pre>
    );
  }
  return (
    ...
    <div
      className="my-5 flex justify-center overflow-x-auto rounded-xl border border-border bg-card p-4"
