...
function ReaderActions({
  ...
  const [sentiment, setSentiment] = useState<"helpful" | "not_helpful" | null>(
    null,
  );
  ...
  const editUrl =
    addons?.editSuggestions !== false
      ? applyUrlTemplate(addons?.editUrl, path, pageUrl)
      : null;
  const issueUrl =
    addons?.issueLinks !== false
      ? applyUrlTemplate(addons?.issueUrl, path, pageUrl)
      : null;
  ...
  return (
    <div
      className={cn(
        ...
        presentation === "card"
          ? "mt-8 rounded-xl border border-border bg-muted/30 p-4 sm:p-5"
          : "mt-14 border-border/70 border-t pt-6",
      )}
    >
      {showFeedback ? (
...
export function SitePageView({
  ...
  const resolvedVersion = data.versions.find(
    (item) => item.slug === data.activeVersion,
  );
  const versionPrefix =
    resolvedVersion && !resolvedVersion.isDefault
      ? resolvedVersion.slug
      : undefined;
  ...
  const showToc =
    mode === "default" && !page.config?.hideToc && page.headings.length > 0;
  ...
  const readerActionsAfterNavigation =
    addons?.feedbackPlacement === "after-navigation";
  ...
  const readingMinutes = Math.max(
    1,
    Math.ceil(readableText.split(/\s+/).filter(Boolean).length / 220),
  );
  const imageCount = (page.content.match(/!\[[^\]]*\]\([^)]*\)|<img\b/gi) ?? [])
    .length;
  const updatedLabel = new Intl.DateTimeFormat(language || "en", {
    dateStyle: "medium",
  ...
  const article = (
    ...
    >
        ...
        <div className="mb-3 flex flex-wrap items-center gap-1.5 font-semibold text-primary text-sm">
          {ancestors.map((crumb, index) => (
            <span key={crumb.path} className="flex items-center gap-1.5">
              ...
              <a
                href={siteHref(projectId, crumb.path, {
                  lang,
      ...
      <ul
        className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-muted-foreground text-xs"
        aria-label={tArticle("articleDetails")}
      >
        <li className="inline-flex items-center gap-1.5">
          <Clock3 className="size-3.5" aria-hidden /> {readingMinutes}{" "}
          {tArticle("minRead")}
        </li>
        {imageCount > 0 ? (
          <li className="inline-flex items-center gap-1.5">
            <Image className="size-3.5" aria-hidden /> {imageCount}{" "}
            {tArticle(imageCount === 1 ? "screenshot" : "screenshots")}
          </li>
        ) : null}
        <li className="inline-flex items-center gap-1.5">
          <CalendarClock className="size-3.5" aria-hidden />{" "}
          {tArticle("updated")} {updatedLabel}
        </li>
      </ul>
      {page.config?.tags?.length ? (
        <ul className="mt-4 flex flex-wrap gap-2" aria-label={tArticle("tags")}>
          {page.config.tags.map((tag) => (
            <li
              className="rounded-full border border-border bg-muted/45 px-2.5 py-1 font-medium text-muted-foreground text-xs"
              key={tag}
            >
              {tag}
            </li>
      ...
      <div className="mt-8">
        <Markdown
          ...
          site={{ projectId, lang: language, version: versionPrefix }}
        />
      </div>

      {readerActionsAfterNavigation ? null : (
        <ReaderActions
          projectId={projectId}
          path={page.path}
          language={language}
      ...
      {prev || next ? (
        <nav
          className={cn(
            ...
            hasReaderActions && !readerActionsAfterNavigation
              ? "mt-8"
              : "mt-14 border-border/70 border-t pt-6",
          )}
        ...
        >
          {prev ? (
            <a
              href={siteHref(projectId, prev.path, {
                lang,
                version: versionPrefix,
              })}
          ...
          {next ? (
            <a
              href={siteHref(projectId, next.path, {
                lang,
                version: versionPrefix,
              })}
  ...
  if (mode === "center") {
    return (
      <div className="mx-auto min-w-0 max-w-[42rem] py-9 lg:py-12">
        {article}
      </div>
    );
  }
