...
import type { LimitState, UsageAvailability } from "@cms/usage";
import {
  ArrowDownToLine,
  ArrowUpFromLine,
  Bot,
...
const meterPresentation: Record<
  string,
  { icon: LucideIcon; label: MessageKey }
> = {
  editor_seat: { icon: Users, label: "settings.usage.meter.editorSeat" },
  asset_storage_byte: {
    icon: HardDrive,
    label: "settings.usage.meter.assetStorage",
  },
  ...
  ai_input_token: {
    ...
    label: "settings.usage.meter.aiInputToken",
  },
  ai_output_token: {
    icon: ArrowUpFromLine,
    label: "settings.usage.meter.aiOutputToken",
  },
  embedded_chunk: { icon: Boxes, label: "settings.usage.meter.embeddedChunk" },
  indexed_content_byte: {
    icon: Database,
    label: "settings.usage.meter.indexedContent",
  },
...
const planLabelKeys: ReadonlyMap<string, MessageKey> = new Map([
  ["free", "settings.usage.plan.free"],
]);
...
export function UsageTab({ project }: { project: Project }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <span className="rounded-full border border-border bg-background px-3 py-1.5 font-medium text-sm">
            {t(
              planLabelKeys.get(usage?.plan.key ?? "") ??
                "settings.usage.plan.unconfigured",
            )}
          </span>
        </div>
            ...
            {usage
              ? t("settings.usage.period", {
                  start: date(usage.period.start),
                    ...
                    new Date(new Date(usage.period.endExclusive).getTime() - 1),
                  ),
                })
              : t("settings.usage.period.loading")}
          </span>
          <span aria-live="polite">
            {usage ? t(availabilityKeys[usage.availability]) : null}
          </span>
        </div>
      </header>
      ...
      {groups.map((group) => {
        const metrics =
          usage?.meters.filter((metric) => group.keys.includes(metric.key)) ??
          [];
        return (
          <SettingsSection
                    ...
                    const formatQuantity = (value: string) => {
                      ...
                      return formatByteQuantity(value, {
                        ...
                        decimal: (whole, fraction) =>
                          t("settings.usage.decimal", { whole, fraction }),
                        unit: (valueLabel, unit) =>
                          t("settings.usage.unit", {
                            value: valueLabel,
                            unit: t(
                              unit === "gib"
                                ? "settings.usage.unit.gib"
                                : unit === "mib"
                                  ? "settings.usage.unit.mib"
                                  : "settings.usage.unit.byte",
                            ),
                          }),
                      });
                    };
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={cn(
                              ...
                              metric.state === "exceeded" &&
                                "bg-destructive/10 text-destructive",
                              metric.state === "approaching" &&
                          ...
                          >
                            {metric.availability === "partial"
                              ? t("settings.usage.state.partial")
                              : t(stateKeys[metric.state])}
                          </span>
                        </div>
                          ...
                          <span
                            ...
                            title={
                              metric.quantity !== null && metric.unit === "byte"
                                ? t("settings.usage.exactBytes", {
                                    value: number(BigInt(metric.quantity)),
                                  })
                                : undefined
                            }
                          >
                            {metric.quantity === null
                              ? "—"
                              : formatQuantity(metric.quantity)}
                          </span>
                          {metric.limit ? (
                            <span className="text-muted-foreground text-sm tabular-nums">
                              / {formatQuantity(metric.limit)}
                            </span>
                          ) : null}
                        </div>
                        {metric.limit && !unavailable ? (
                          <div
                            aria-label={t("settings.usage.progressLabel", {
                        ...
                        {unavailable ? (
                          <p className="mt-2 text-muted-foreground text-xs">
                            {t("settings.usage.unknownExplanation")}
                          </p>
                        ) : null}
                      </article>
                    );
      ...
      <p className="rounded-lg border border-border bg-muted/30 p-3 text-muted-foreground text-xs">
