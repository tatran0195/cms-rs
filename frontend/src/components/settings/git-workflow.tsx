...
import type { GitOperationBody } from "@cms/validators";
import {
  AlertTriangle,
  ArrowLeft,
  ArrowRight,
...
function ConflictCard({
  ...
  const resolveConflict = (
    ...
    resolve.mutate(
      {
        resolution,
        ...(resolution === "CUSTOM" ? { content: content ?? null } : {}),
      },
      {
        onError: (error) =>
          toast.error(
            error instanceof Error
              ? error.message
              : t("settings.git.workflow.conflict.error"),
          ),
      },
    );
  return (
    <section className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Badge variant="destructive">
          {t("settings.git.workflow.conflict.badge")}
        </Badge>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <Button
          ...
          size="sm"
        >
          {t("settings.git.workflow.conflict.useCMS")}
        </Button>
        <Button
          disabled={resolve.isPending}
        ...
        <Button
          disabled={resolve.isPending}
          onClick={() => resolveConflict("CUSTOM", custom)}
          size="sm"
          variant="secondary"
        >
        ...
        <Button
          disabled={resolve.isPending}
          onClick={() => resolveConflict("CUSTOM", null)}
          size="sm"
          variant="destructive"
        >
...
export function GitWorkflow({ projectId }: { projectId: string }) {
  ...
  const [intent, setIntent] = useState<
    "overview" | "sync" | "publish" | "connection"
  >("overview");
      ...
      },
      {
        ...
        onError: (error) =>
          toast.error(
            error instanceof Error
              ? error.message
              : t("settings.git.workflow.connectError"),
          ),
      },
    );
    ...
    operation.mutate(body, {
      onError: (error) =>
        toast.error(
          error instanceof Error
            ? error.message
            : t("settings.git.workflow.operationError"),
        ),
    });
  const activeConflicts = useMemo(
    () =>
      connection?.operations.flatMap((item) =>
        item.conflicts.filter((conflict) => conflict.status === "OPEN"),
      ) ?? [],
    [connection],
  );
  const webhookUrl = new URL(
    `/api/public/git/webhook/${projectId}`,
    env.VITE_APP_URL,
  ).toString();
  ...
  if (query.isPending)
    return (
      <div
        aria-live="polite"
        className="rounded-xl border p-5 text-muted-foreground text-sm"
      >
        ...
        <AlertDescription className="flex flex-wrap items-center justify-between gap-3">
          <span>
            {query.error instanceof Error
              ? query.error.message
              : t("settings.git.workflow.loadErrorHint")}
          </span>
    ...
    return (
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <h2 className="font-semibold text-base">
                {t("settings.git.workflow.connectTitle")}
              </h2>
              <p className="max-w-2xl text-muted-foreground text-sm leading-6">
                {t("settings.git.workflow.connectDescription")}
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="space-y-6 p-5 sm:p-6">
          <ol
            aria-label={t("settings.git.workflow.progress")}
            className="grid gap-3 sm:grid-cols-3"
          >
            {(
              [
                [
                  t("settings.git.workflow.step.authorize"),
                  t("settings.git.workflow.step.authorizeDesc"),
                ],
                [
                  t("settings.git.workflow.step.repository"),
                  t("settings.git.workflow.step.repositoryDesc"),
                ],
                [
                  t("settings.git.workflow.step.review"),
                  t("settings.git.workflow.step.reviewDesc"),
                ],
              ] as const
          ...
          <div className="rounded-xl border border-border bg-background p-5 shadow-sm sm:p-6">
              ...
              <div className="max-w-2xl space-y-5">
                <div className="space-y-1.5">
                  <h3 className="font-medium text-sm">
                    {t("settings.git.workflow.authorizeTitle")}
                  </h3>
                  <p className="text-muted-foreground text-sm leading-6">
                    {t("settings.git.workflow.authorizeDescription")}
                  </p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="git2-token">
                    {t("settings.git.workflow.token")}
                  </Label>
                  <Input
                    autoComplete="off"
                    id="git2-token"
                ...
                {authorize.isError ? (
                  ...
                  >
                    {authorize.error instanceof Error
                      ...
                      : t("settings.git.workflow.authorizeError")}
                  </p>
                ) : null}
              </div>
            ) : null}
            {connectStep === 2 ? (
              <div className="space-y-5">
                <div className="space-y-1.5">
                  <h3 className="font-medium text-sm">
                    {t("settings.git.workflow.chooseTitle")}
                  </h3>
                  <p className="text-muted-foreground text-sm leading-6">
                    {t("settings.git.workflow.chooseDescription")}
                  </p>
                </div>
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="git2-repo">
                      {t("settings.git.repository.title")}
                    </Label>
                    <Input
                      id="git2-repo"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="git2-base">
                      {t("settings.git.workflow.baseBranch")}
                    </Label>
                    <Input
                      id="git2-base"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="git2-head">
                      {t("settings.git.workflow.headBranch")}
                    </Label>
                    <Input
                      id="git2-head"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="git2-path">
                      {t("settings.git.contentPath")}
                    </Label>
                    <Input
                      id="git2-path"
            ...
            {connectStep === 3 ? (
              <div className="space-y-5">
                <div className="space-y-1.5">
                  <h3 className="font-medium text-sm">
                    {t("settings.git.workflow.reviewTitle")}
                  </h3>
                  <p className="text-muted-foreground text-sm leading-6">
                    {t("settings.git.workflow.reviewDescription")}
                  </p>
                </div>
                {authorizedAccount ? (
                  <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3 text-sm">
                    <span>
                      {t("settings.git.workflow.authorizedAs")}{" "}
                      <strong>@{authorizedAccount.login}</strong>
                    ...
                    <Badge variant="outline">
                      {t("settings.git.workflow.verified")}
                    </Badge>
                  </div>
                ) : null}
                ...
                <p className="text-muted-foreground text-xs leading-5">
                  {t("settings.git.workflow.reviewHint")}
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div className="flex flex-col-reverse gap-3 border-border border-t pt-5 sm:flex-row sm:items-center sm:justify-between">
            <Button
              ...
              onClick={() =>
                setConnectStep((step) => Math.max(1, step - 1) as 1 | 2 | 3)
              }
              variant="outline"
            >
            ...
            {connectStep < 3 ? (
              connectStep === 1 ? (
                ...
                >
                  {authorize.isPending
                    ? t("settings.git.workflow.authorizing")
                    : t("settings.git.workflow.authorize")}{" "}
                ...
                >
                  {t("settings.git.workflow.review")}{" "}
                  <ArrowRight className="size-4" />
                </Button>
              )
            ) : (
              ...
              >
                {connect.isPending
                  ? t("settings.git.workflow.verifying")
                  : t("settings.git.workflow.connect")}
              </Button>
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold">
                {t("settings.git.workflow.title")}
              </h2>
              <Badge variant={statusTone(connection.lastSyncStatus)}>
                {connection.lastSyncStatus}
              </Badge>
            </div>
            <p className="mt-1 font-mono text-sm">
              {connection.repository}: {connection.headBranch} →{" "}
      ...
      <nav
        aria-label={t("settings.git.workflow.actions")}
      ...
      >
        {(
          [
            [
              "overview",
              t("settings.git.workflow.nav.overview"),
              t("settings.git.workflow.nav.overviewDesc"),
            ],
            [
              "sync",
              t("settings.git.workflow.nav.sync"),
              t("settings.git.workflow.nav.syncDesc"),
            ],
            [
              "publish",
              t("settings.git.workflow.nav.publish"),
              t("settings.git.workflow.nav.publishDesc"),
            ],
            [
              "connection",
              t("settings.git.workflow.nav.connection"),
              t("settings.git.workflow.nav.connectionDesc"),
            ],
          ] as const
        ...
        <section className="space-y-3">
          <div>
            <h3 className="font-semibold">
              {t("settings.git.workflow.conflict.title")}
            </h3>
            <p className="text-muted-foreground text-sm">
              {t("settings.git.workflow.conflict.description")}
            </p>
          </div>
      ...
      {intent === "sync" ? (
        <section className="rounded-xl border p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold">
              {t("settings.git.workflow.syncTitle")}
            </h3>
          </div>
          <p className="mt-1 text-muted-foreground text-sm">
            {t("settings.git.workflow.syncDescriptionBefore")}{" "}
            <code>{connection.baseBranch}</code>{" "}
            {t("settings.git.workflow.syncDescriptionAfter")}
          </p>
          <Button
            className="mt-4"
            ...
            onClick={() =>
              queueOperation({
                ...
                kind: "PULL",
                sourceRef: connection.baseBranch,
              })
            }
          >
            <RefreshCw
              className={`size-4 ${operation.isPending ? "animate-spin" : ""}`}
            />{" "}
            {connection.lastSyncedAt
              ? t("settings.git.workflow.syncLatest")
              : t("settings.git.workflow.importExisting")}
          </Button>
          {activeConflicts.length > 0 ? (
            <p className="mt-2 text-amber-700 text-xs dark:text-amber-300">
              {t("settings.git.workflow.resolveFirst")}
            </p>
          ) : null}
        </section>
      ) : null}
      ...
      {intent === "publish" ? (
        <section className="rounded-xl border p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold">
              {t("settings.git.workflow.publishTitle")}
            </h3>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <Label htmlFor="git2-message">
                {t("settings.git.workflow.commitMessage")}
              </Label>
              <Input
                id="git2-message"
            ...
            <div>
              <Label htmlFor="git2-author">
                {t("settings.git.workflow.authorName")}
              </Label>
              <Input
                id="git2-author"
            ...
            <div>
              <Label htmlFor="git2-email">
                {t("settings.git.workflow.authorEmail")}
              </Label>
              <Input
                id="git2-email"
            ...
            <div className="md:col-span-2">
              <Label htmlFor="git2-title">
                {t("settings.git.workflow.prTitle")}
              </Label>
              <Input
                id="git2-title"
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            >
              ...
              {t("settings.git.workflow.updatePr")}
            </Button>
          </div>
        </section>
      ) : null}
      ...
      {intent === "overview" || intent === "publish" ? (
        <section className="rounded-xl border p-5">
          <h3 className="font-semibold">
            {t("settings.git.workflow.previewsTitle")}
          </h3>
              ...
              connection.pullRequests.map((pull) => {
                ...
                return (
                  <div
                    ...
                    key={pull.id}
                  >
                    <Badge variant={pull.draft ? "secondary" : "outline"}>
                      {pull.draft
                        ? t("settings.git.workflow.draft")
                        : pull.state}
                    </Badge>
                    <a
                    ...
                    <span className="ms-auto text-muted-foreground text-xs">
                      {t("settings.git.workflow.preview")}:{" "}
                      {preview?.status ?? t("settings.git.workflow.pending")}
                    </span>
                    {preview?.url ? (
                      <a
      ...
      {intent === "connection" ? (
        <>
          <section className="rounded-xl border p-5">
            ...
            <p className="text-muted-foreground text-sm">
              {t("settings.git.workflow.webhookDescription")}
            </p>
                ...
                rotate.mutate(undefined, {
                  ...
                  onError: (error) =>
                    toast.error(
                      error instanceof Error
                        ? error.message
                        : t("settings.git.workflow.rotateError"),
                    ),
                })
              }
              size="sm"
          ...
          <section className="rounded-xl border p-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold">
                {t("settings.git.workflow.configuration")}
              </h3>
            </div>
            <p className="text-muted-foreground text-sm">
              {t("settings.git.workflow.configurationDescription")}
            </p>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div>
                <Label htmlFor="git2-update-repo">
                  {t("settings.git.repository.title")}
                </Label>
                <Input
                  id="git2-update-repo"
              ...
              <div>
                <Label htmlFor="git2-update-token">
                  {t("settings.git.workflow.rotateToken")}
                </Label>
                <Input
                  autoComplete="off"
                  id="git2-update-token"
              ...
              <div>
                <Label htmlFor="git2-update-base">
                  {t("settings.git.workflow.baseBranch")}
                </Label>
                <Input
                  id="git2-update-base"
              ...
              <div>
                <Label htmlFor="git2-update-head">
                  {t("settings.git.workflow.headBranch")}
                </Label>
                <Input
                  id="git2-update-head"
              ...
              <div>
                <Label htmlFor="git2-update-path">
                  {t("settings.git.contentPath")}
                </Label>
                <Input
                  id="git2-update-path"
            ...
            <div className="mt-4 flex flex-wrap justify-between gap-2">
              <Button
                ...
                onClick={() => {
                  if (
                    window.confirm(t("settings.git.workflow.disconnectConfirm"))
                  ) {
                    disconnect.mutate(undefined, {
                      ...
                      onError: (error) =>
                        toast.error(
                          error instanceof Error
                            ? error.message
                            : t("settings.git.workflow.disconnectError"),
                        ),
                    });
                  }
                }}
              ...
              >
                {connect.isPending
                  ? t("common.saving")
                  : t("settings.git.workflow.saveConnection")}
              </Button>
            </div>
          </section>
        </>
      ...
      {intent === "overview" || intent === "sync" ? (
        <section className="rounded-xl border p-5">
          <h3 className="font-semibold">
            {t("settings.git.workflow.recentOperations")}
          </h3>
          <div className="mt-3 space-y-2">
            {connection.operations.map((item) => (
              <div className="rounded-lg border p-3" key={item.id}>
                ...
                {item.error ? (
                  <p className="mt-2 text-destructive text-xs">{item.error}</p>
                ) : null}
              </div>
            ))}
          </div>
