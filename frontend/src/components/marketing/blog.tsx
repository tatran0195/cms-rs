...
const dateFormatter = (entry: BlogEntry) =>
  new Intl.DateTimeFormat(blogLanguage(entry) === "ar" ? "ar" : "en-US", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
...
const gradientFor = (slug: string) =>
  GRADIENTS[
    [...slug].reduce((sum, ch) => sum + ch.charCodeAt(0), 0) % GRADIENTS.length
  ];
...
function CoverPanel({
  ...
  return (
    <div
      className={`${className} relative flex items-center justify-center overflow-hidden bg-gradient-to-br ${gradientFor(entry.slug)}`}
    >
      <div
  ...
  return (
    <div className="mt-auto flex flex-wrap items-center gap-x-2 gap-y-1 text-muted-foreground text-xs">
      <time dateTime={entry.datePublished}>
        {dateFormatter(entry).format(new Date(entry.datePublished))}
      </time>
      ...
      <span className="inline-flex items-center gap-1">
        ...
        {translateFn(
          "blog.readingMinutes",
          { minutes: blogReadingMinutes(entry) },
          language,
        )}
      </span>
    </div>
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-1.5">
      {entry.tags.slice(0, 3).map((tag) => (
        <span
          key={tag}
          className="rounded-full border border-border bg-background px-2.5 py-0.5 font-medium text-muted-foreground text-xs"
        >
          {tag}
        </span>
      ))}
  ...
  if (featured) {
    return (
      <article dir={direction} lang={language}>
        ...
        >
          <CoverPanel
            className="aspect-[16/10] w-full md:aspect-auto md:h-full md:min-h-[260px]"
            entry={entry}
          />
          <div className="flex flex-col justify-center p-7 md:p-9">
            ...
            <p className="mt-3 mb-5 max-w-[48ch] text-muted-foreground text-sm leading-relaxed sm:text-[15px]">
              {entry.description}
            </p>
    ...
    <article className="h-full" dir={direction} lang={language}>
      ...
      >
        ...
        <div className="flex flex-1 flex-col p-6">
          ...
          <p className="mt-2 mb-4 line-clamp-3 text-muted-foreground text-sm leading-relaxed">
            {entry.description}
          </p>
          <CardMeta entry={entry} />
        </div>
...
function Callout({ children, title }: { children: ReactNode; title?: string }) {
  return (
    <aside className="my-6 rounded-xl border border-border border-s-2 border-s-primary bg-card px-5 py-4">
      {title ? (
        <p className="mb-1 font-semibold text-foreground text-sm">{title}</p>
      ) : null}
      <div className="text-muted-foreground text-sm leading-relaxed [&>p:last-child]:mb-0 [&>p]:mb-2">
        {children}
      </div>
    </aside>
  );
      ...
      <dl className="flex flex-col gap-5">
        {faqs.map((faq) => (
          <div key={faq.question}>
            ...
            <dd className="ms-0 text-muted-foreground text-sm leading-relaxed">
              {faq.answer}
            </dd>
          </div>
        ))}
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-foreground px-8 py-14 text-center text-background">
        ...
        <div className="relative">
          <h2 className="font-semibold text-3xl tracking-tight rtl:tracking-normal">
            {translateFn("blog.ctaTitle", undefined, language)}
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-background/75">
            {translateFn("blog.ctaBody", undefined, language)}
          </p>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            <a className={primaryButton} href="/sign-up">
              {translateFn("blog.createAccount", undefined, language)}{" "}
...
function ShareRow({ entry }: { entry: BlogEntry }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="me-1 text-muted-foreground text-xs">
        {translateFn("blog.share", undefined, language)}
      </span>
      {canCopy ? (
        <button
          aria-label={translateFn(
            copied ? "blog.linkCopied" : "blog.copyLink",
            undefined,
            language,
          )}
      ...
      >
        <span className="sr-only">
          {translateFn("blog.shareX", undefined, language)}
        </span>
        <svg
          aria-hidden="true"
          className="size-4"
      ...
      >
        <span className="sr-only">
          {translateFn("blog.shareLinkedIn", undefined, language)}
        </span>
        <svg
          aria-hidden="true"
          className="size-4"
...
export function ArticlePage({
  ...
  const related = (entry.related ?? [])
    .map(blogEntry)
    .filter((item): item is BlogEntry => Boolean(item));
  const language = blogLanguage(entry);
    ...
    <MarketingShell stars={stars}>
      ...
      >
        ...
        >
          ...
          {translateFn("blog.allArticles", undefined, language)}
        </a>
        <header className="mt-6">
          <TagChips entry={entry} />
          <h1
            className={`text-pretty font-semibold text-4xl leading-[1.15] sm:text-[44px] ${arabic ? "" : "tracking-tight"}`}
          >
            {entry.title}
          </h1>
          <p className="mt-4 max-w-2xl text-lg text-muted-foreground leading-relaxed">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-border border-y py-4">
            <p className="text-muted-foreground text-sm">
              <time className="datePublished" dateTime={entry.datePublished}>
                {translateFn("blog.published", undefined, language)}{" "}
                {dateFormatter(entry).format(new Date(entry.datePublished))}
              </time>
              {entry.dateModified !== entry.datePublished ? (
                <>
                  ...
                  <time dateTime={entry.dateModified}>
                    {translateFn("blog.updated", undefined, language)}{" "}
                    {dateFormatter(entry).format(new Date(entry.dateModified))}
                  </time>
                </>
              ) : null}
              {" · "}
              ...
              <a
                className="author font-medium text-foreground hover:underline"
                href="/about"
                rel="author"
              >
        ...
        {entry.faqs && entry.faqs.length > 0 ? (
          <ArticleFaqSection faqs={entry.faqs} language={language} />
        ) : null}
      </article>
      {related.length > 0 ? (
        <aside className="mx-auto max-w-6xl px-6 pb-16">
          <h2
            className={`mb-5 font-semibold text-xl ${arabic ? "" : "tracking-tight"}`}
          >
            {translateFn("blog.readNext", undefined, language)}
          </h2>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((item) => (
