...
import { cmsPricing } from "@/lib/comparison-data";

/**
 * Shared page templates for the /compare/cms-vs-* and /alternatives/*
  ...
  return (
    <MarketingShell stars={stars}>
      <PageHero
        ...
        paragraphs={data.directAnswer}
        asOf={data.competitorPricing.asOf}
      />
      {data.slug === "cms-vs-gitbook" || data.slug === "cms-vs-mintlify" ? (
        <GitbookMintlifyGuideLink />
      ) : null}
      ...
      <FeatureMatrix
        ...
        rows={data.features}
      />
      <WhenToPick pickCompetitor={data.pickCompetitor} pickCMS={data.pickCMS} />
  ...
  return (
    <MarketingShell stars={stars}>
      <PageHero
        ...
        asOf={data.competitorPricing.asOf}
      />
      {data.slug === "gitbook" || data.slug === "mintlify" ? (
        <GitbookMintlifyGuideLink />
      ) : null}
      <section className="mx-auto max-w-4xl px-6 py-20">
        ...
        <ol className="mt-10 space-y-6">
          {data.alternatives.map((alt, i) => (
            <li
              key={alt.name}
              className={`rounded-xl border bg-card p-7 ${alt.isCMS ? "border-primary/30 ring-1 ring-primary/20" : "border-border"}`}
            >
              <div className="flex flex-wrap items-center gap-3">
                ...
                <h3 className="flex items-center gap-2 font-semibold text-lg tracking-tight">
                  {alt.isCMS ? <CMSMark className="size-5" /> : null}
                  {alt.name}
                </h3>
                {alt.isCMS ? (
      ...
      <PricingSection
        competitor={data.competitorPricing}
        headingPrefix={`What ${data.competitorName} costs`}
      />
      <FaqSection faqs={data.faqs} />
      <MarketingCta />
    </MarketingShell>
  ...
  return (
    <section className="border-border border-b">
      <div className="mx-auto max-w-3xl px-6 py-20">
        ...
        <h1 className="mt-4 text-balance text-center font-semibold text-4xl tracking-tight sm:text-5xl">
          {heading}
        </h1>
        {paragraphs.map((p, i) => (
          <p
        ...
        <p className="mx-auto mt-6 max-w-2xl border-border border-t pt-4 text-muted-foreground text-xs leading-relaxed">
          We build CMS, so read this page as an informed but interested party:
          every price was checked against the vendor’s official pricing page as
          of {asOf} and links to its source, and everything CMS doesn’t do yet
          is disclosed plainly.
        </p>
      </div>
    </section>
  );
      ...
      <div className="mx-auto max-w-6xl px-6 py-20">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 font-semibold text-3xl tracking-tight">
            {headingPrefix ?? `${competitor.productName} pricing`} vs CMS
          </h2>
          <p className="mt-3 text-lg text-muted-foreground">
            Numbers below are from the official pricing pages as of{" "}
            {competitor.asOf} — always check the linked source for current
            figures.
          </p>
        </div>
        <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-2">
          <PricingTableView table={competitor} />
          <PricingTableView table={cmsPricing} />
        </div>
      </div>
    </section>
  );
        ...
        <table className="w-full table-fixed text-start text-sm sm:min-w-[26rem] sm:table-auto">
          ...
          <thead>
            <tr className="border-border border-b bg-muted/40 text-muted-foreground">
              ...
              <th className="px-2 py-3 text-start font-medium sm:px-4">
                Price
              </th>
              <th className="px-2 py-3 text-start font-medium sm:px-4">
                What you get
          ...
          <tbody>
            {table.rows.map((row) => (
              ...
              >
                ...
                <td className="px-2 py-3 sm:px-4 sm:whitespace-nowrap">
                  {row.price}
                </td>
                <td className="px-2 py-3 text-muted-foreground leading-relaxed sm:px-4">
                  {row.includes}
                </td>
              </tr>
            ))}
  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-20" id="features">
      <div className="max-w-2xl">
        ...
        <h2 className="mt-4 font-semibold text-3xl tracking-tight">
          CMS vs {competitorName}, feature by feature
        </h2>
      </div>
      <div className="mt-10 overflow-x-auto rounded-xl border border-border bg-card shadow-xs">
        <table className="w-full table-fixed text-sm sm:min-w-[34rem] sm:table-auto">
          <caption className="sr-only">
            CMS and {competitorName} feature comparison
          </caption>
          <thead>
            <tr className="border-border border-b bg-muted/40">
              <th className="px-2 py-3 text-start font-medium text-muted-foreground sm:px-5">
                Feature
              </th>
      ...
      <p className="mt-4 text-muted-foreground text-xs leading-relaxed">
        Items marked “Not yet” are documented gaps without a committed delivery
        date. Review the current{" "}
        <a
          className="underline underline-offset-2 hover:text-foreground"
          href="/self-hosting"
        >
        ...
        . “—” means the vendor’s pricing page does not state it either way;
...
function FeatureCellView({
  ...
  const icon: Record<string, ReactNode> = {
    yes: (
      <Check
        className={`size-4 ${muted ? "text-muted-foreground" : "text-primary"}`}
        aria-hidden="true"
      />
    ),
  ...
  return (
    <div className="flex flex-col items-center gap-1 text-center">
      {icon[cell.value] ? (
        ...
        <span
          className={`font-medium text-xs ${cell.value === "planned" ? "text-foreground" : "text-muted-foreground"}`}
        >
          {label[cell.value]}
        </span>
      )}
    ...
    <section className="border-border border-y bg-card/40" id="when">
      <div className="mx-auto max-w-6xl px-6 py-20">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-lg text-muted-foreground">
            Both are legitimate choices — it depends on what your team needs
            today.
          </p>
        </div>
        <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <PickCard pick={pickCompetitor} />
          <PickCard pick={pickCMS} featured />
        </div>
      </div>
    </section>
  );
...
function PickCard({
  ...
  return (
    <div
      className={`rounded-xl border bg-card p-7 ${featured ? "border-primary/30 ring-1 ring-primary/20" : "border-border"}`}
    >
    ...
    <section className="mx-auto max-w-5xl px-6 pb-24">
      <div className="relative overflow-hidden rounded-2xl border border-border bg-foreground px-8 py-14 text-center text-background">
        ...
        <div className="relative">
          <h2 className="font-semibold text-3xl tracking-tight">
            Try CMS for yourself
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-background/75">
            Start free on CMS Cloud with no credit card, or review the
