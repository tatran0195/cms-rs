...
export function PublishModal({
  ...
  const {
    data: pending,
    isPending: loadingChanges,
    isError: preflightFailed,
  } = usePendingChanges(project.id, { enabled: open });
  ...
  const doPublish = () => {
    if (
      loadingChanges ||
      preflightFailed ||
      (pending?.redirectIssues?.length ?? 0) > 0
    ) {
      return;
    }
  ...
  const publishBlocked =
    loadingChanges || preflightFailed || redirectIssues.length > 0;
  const hasBaseline = pending?.hasBaseline ?? true;
  const changeKey = useCallback(
    (change: PendingChange) => `${change.id}:${change.status}`,
    [],
  );
  const selectedChange =
    sorted.find((change) => changeKey(change) === selectedKey) ??
    sorted[0] ??
    null;
  ...
  useEffect(() => {
    ...
    if (
      first &&
      (!selectedKey ||
        !sorted.some((change) => changeKey(change) === selectedKey))
    ) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          "gap-0 p-0",
          reviewing ? "sm:max-w-[940px]" : "sm:max-w-[480px]",
        )}
      ...
      >
        ...
        {reviewing && selectedChange ? (
              ...
              <ScrollArea className="h-[470px]">
                <ul className="p-2">
                  {sorted.map((change) => {
                    ...
                    const active =
                      changeKey(change) === changeKey(selectedChange);
                    return (
              ...
              <div className="border-white/10 border-b px-7 py-5">
                <div className="flex items-center gap-2 text-white/60 text-xs">
                  ...
                  <span
                    className={cn(
                      "ms-auto font-medium",
                      STATUS_META[selectedChange.status].chip,
                    )}
                  >
                    {t(STATUS_META[selectedChange.status].labelKey)}
                  </span>
                </div>
                <h2 className="mt-2 max-w-2xl font-semibold text-2xl tracking-tight">
                  {selectedChange.title || selectedChange.path}
                </h2>
                <div className="mt-2 flex gap-3 text-xs">
                  ...
                  {selectedChange.fields.length > 0 ? (
                    <span className="text-white/45">
                      {selectedChange.fields.join(", ")}
                    </span>
                  ) : null}
                </div>
              </div>
                ...
                <div className="px-7 py-6">
                  <div className="overflow-hidden rounded-md border border-white/10 bg-black/25 font-mono text-[12.5px] leading-6">
                    {selectedChange.lines.length > 0 ? (
                        ...
                        >
                          ...
                          <span
                            className={cn(
                              ...
                              line.type === "removed" &&
                                "line-through decoration-rose-200/70",
                            )}
                          >
                            {line.text || " "}
                          </span>
                      ...
                      <div className="px-3 py-8 text-center text-white/45">
                        {t("publish.noLineDiff")}
                      </div>
                    )}
                    {selectedChange.truncated ? (
                      <div className="border-white/10 border-t px-3 py-2 text-white/45">
                        {t("publish.diffTruncated")}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-4 px-6 py-5">
            <div className="flex items-center gap-3 rounded-xl border border-border px-4 py-3">
              <CMSMark className="size-9 shrink-0" />
            ...
            {redirectIssues.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-2 font-semibold text-destructive text-sm">
                  ...
                  {t("publish.redirectsBlocked")}
                </div>
                <p className="mt-1 text-muted-foreground text-xs">
                  {t("publish.redirectsBlockedHint")}
                </p>
                <ul className="mt-3 space-y-1.5 text-xs">
                  {redirectIssues.map((issue) => (
                    <li
                      className="font-mono"
                      key={`${issue.code}-${issue.rowIndexes.join("-")}-${issue.sequence.join("-")}-${issue.message}`}
                    >
                      {issue.message}
                    </li>
                  ))}
            ...
            {preflightFailed ? (
              ...
              >
                <div className="flex items-center gap-2 font-semibold text-destructive text-sm">
                  <TriangleAlert className="size-4" />{" "}
                  {t("publish.preflightFailed")}
                </div>
                <p className="mt-1 text-muted-foreground text-xs">
                  {t("publish.preflightFailedHint")}
                </p>
              </div>
            ) : null}
                  ...
                  <span className="text-muted-foreground text-xs">
                    {hasBaseline
                      ? pending?.lastVersion != null
                        ? t("publish.sinceVersion", {
                            version: pending.lastVersion,
                          })
                        : null
        ...
        {!reviewing ? (
          <DialogFooter className="gap-2.5 px-6 pt-0 pb-5 sm:justify-stretch">
            <DialogClose
              render={
                <Button variant="outline" className="h-[42px] flex-none px-4" />
              }
            >
              {t("common.cancel")}
            </DialogClose>
