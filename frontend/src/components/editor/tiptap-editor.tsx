...
export function buildEditorExtensions({
  ...
  return [
    ...
    SlashCommand.configure({
      onUpload: (file: File) => onUpload?.(file) ?? Promise.resolve(null),
    }),
    CommentDecorations.configure({
      getComments: getComments ?? (() => []),
      getActiveId: getActiveId ?? (() => null),
    }),
  ];
}
...
function EditorFooter({ editor }: { editor: Editor }) {
  ...
  const counts = useEditorState({
    editor,
    selector: ({ editor: current }) => getCounts(current),
  });
  return (
...
export function TiptapEditor({
  ...
  return (
    <div
      className={cn("pl-editor", commentMode && "is-comment-mode", className)}
      dir={dir}
      style={style}
    >
