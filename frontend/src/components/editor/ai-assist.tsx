...
export function AiAssist({
  ...
  const [preview, setPreview] = useState<{ mode: AiMode; text: string } | null>(
    null,
  );
      ...
      },
      {
        onSuccess: (result) => {
          ...
          if (APPEND_MODES.has(mode)) {
            ...
            toast.success(
              mode === "outline"
                ? t("editor.ai.outlineAdded")
                : t("editor.ai.sectionDrafted"),
            );
          } else {
        ...
        onError: (e) =>
          toast.error(
            e instanceof Error ? e.message : t("editor.ai.draftError"),
          ),
          ...
          <div className="space-y-2 rounded-lg border border-border bg-muted/40 p-3">
            <div className="flex items-center gap-1.5 font-medium text-muted-foreground text-xs uppercase tracking-wide">
              ...
              {preview.mode === "summarize"
                ? t("editor.ai.summaryLabel")
                : t("editor.ai.suggestionLabel")}
            </div>
            <p className="max-h-40 overflow-y-auto whitespace-pre-wrap text-foreground/90 text-sm leading-relaxed">
              {preview.text}
            </p>
