...
import {
  type Project,
  type ProjectThemeImportResult,
  useExportProjectTheme,
  useImportProjectTheme,
...
const fullPresetTheme = (preset: ThemePresetId): CMSThemeConfig => {
  ...
  return {
    ...
    colors: {
      light: { ...definition.colors.light },
      dark: { ...definition.colors.dark },
    },
...
const initialDraft = (project: Project): ThemeDraft => {
  const resolved = resolveTheme(
    project.config as ThemeOwnedProjectConfig | null,
  );
  return {
...
export function ThemePreview({
  ...
  const t = (key: MessageKey) =>
    translateFn(key, undefined, arabic ? "ar" : "en");
  return (
    <DocumentationThemeProvider
      appearance={mode}
      ...
      <DocumentationStudioPreviewLayout
        header={
          ...
          >
            ...
            >
              <span className="grid size-6 place-items-center rounded-md bg-primary font-semibold text-primary-foreground">
                N
              </span>
              <span className="font-semibold">
                {t("settings.theme.preview.productDocs")}
              </span>
        ...
        navigation={
          <nav aria-label={t("settings.theme.preview.navigation")}>
            <p className="mb-2 px-2 font-semibold">
              {t("settings.theme.preview.start")}
            </p>
            <a
              className="mb-1 block rounded-md bg-primary/10 px-2 py-1.5 font-medium text-primary"
              href="#preview-content"
            >
              {t("settings.theme.preview.overview")}
            </a>
            <a
              className="block rounded-md px-2 py-1.5 text-muted-foreground"
              href="#preview-code"
            >
        ...
        content={
          ...
          >
            <p className="font-medium text-primary text-xs">
              {t("settings.theme.preview.guide")}
            </p>
              ...
              style={
                {
                  ...
                  borderColor:
                    "color-mix(in oklab,var(--theme-info) 38%,transparent)",
                  background:
                    "color-mix(in oklab,var(--theme-info) 10%,transparent)",
                } as CSSProperties
              }
            >
            ...
            >
              ...
              <p className="mt-1 text-muted-foreground">
                {t("settings.theme.preview.publish")}
              </p>
            </div>
          </article>
        }
...
type ThemeOption =
  | ThemeLayout[keyof ThemeLayout]
  | ThemeComponents[keyof ThemeComponents];
...
function ThemePresetThumbnail({ presetId }: { presetId: ThemePresetId }) {
  ...
  if (presetId === "manuscript") {
    return (
      <span
        className="mb-3 flex h-20 flex-col overflow-hidden rounded-lg border"
      ...
      >
        ...
        />
        <span
          className="mx-3 mt-2 h-2 border-y"
  ...
  if (presetId === "signal") {
    return (
      ...
      >
        <span
          className="col-span-2 flex items-center gap-1 border px-1"
        ...
        <span
          ...
          style={{ borderColor: colors.border, background: colors.code }}
        >
  ...
  return (
    <span
      className="mb-3 grid h-20 grid-cols-[28%_1fr_18%] grid-rows-[16px_1fr] overflow-hidden rounded-lg border"
      aria-hidden
    >
      <span
      ...
      >
        <span
          className="m-1.5 block h-1.5 w-10 rounded-full"
          style={{ background: colors.accent }}
        />
      </span>
      <span
...
export function ThemeSection({ project }: { project: Project }) {
  ...
  const [previewMode, setPreviewMode] = useState<PreviewMode>(
    saved.appearance === "dark" ? "dark" : "light",
  );
  ...
  const [importPreview, setImportPreview] =
    useState<ProjectThemeImportResult>();
      ...
      { template, mode: importMode, apply },
      {
        onSuccess: (result) => {
          ...
          if (apply) {
            setDraft({
              theme:
                result.template.config.theme ??
                fullPresetTheme(result.theme.id),
        ...
        onError: (error) =>
          toast.error(
            error instanceof Error
              ? error.message
              : apply
                ? t("settings.theme.import.applyError")
                : t("settings.theme.import.previewError"),
          ),
      },
    );
  };
  ...
  const exportTheme = () =>
    exportProjectTheme.mutate(undefined, {
      onSuccess: ({ json }) => {
        downloadText(`${project.slug || "cms"}-theme.json`, json);
  ...
  const parseImportText = () => {
    if (
      new TextEncoder().encode(importText).byteLength > MAX_THEME_TEMPLATE_BYTES
    ) {
      toast.error(t("settings.theme.import.tooLarge"));
      return;
    }
  ...
  const setThemePart = (
    ...
    change((current) => ({
      ...
      theme: {
        ...current.theme,
        [section]: {
          ...(current.theme[section] as Record<string, unknown> | undefined),
          [key]: value,
        },
      } as CMSThemeConfig,
    }));
  ...
  return (
    <form onSubmit={save}>
      ...
      <div className="mb-6 rounded-xl border border-primary/25 bg-primary/5 p-4 text-sm">
        ...
        <p className="mt-1 text-muted-foreground">
          {t("settings.theme.draftHint")}
        </p>
      </div>

      <Field
        hint={t("settings.theme.galleryHint")}
        label={t("settings.theme.gallery")}
      >
          ...
          {THEME_PRESET_IDS.map((presetId) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  change({
                    theme: fullPresetTheme(presetId),
              ...
              >
                ...
                <span className="mt-1 block text-muted-foreground text-xs leading-relaxed">
                  {t(`settings.theme.preset.${presetId}.description`)}
                </span>
                <span className="mt-2 block text-[11px] text-primary">
                  {t(`settings.theme.preset.${presetId}.rationale`)}
                </span>
              </button>
            );
          })}
      ...
      <Field
        hint={t("settings.theme.appearanceHint")}
        label={t("settings.theme.appearance")}
      >
        <Segmented
          className="max-w-[340px]"
          onChange={(appearance) =>
            change((current) => ({ ...current, appearance }))
          }
          options={[
      ...
      <Field
        hint={t("settings.theme.layoutHint")}
      ...
      <Field
        hint={t("settings.theme.componentsHint")}
        label={t("settings.theme.components")}
      >
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <NativeSelect
      ...
      <Field
        hint={t("settings.theme.colorsHint")}
        label={t("settings.theme.colors")}
      >
        <Segmented
        ...
        {contrastIssues.length > 0 ? (
          <div
            className="mt-3 rounded-lg border border-destructive/35 bg-destructive/8 p-3 text-destructive text-xs"
            role="alert"
          >
            ...
            <ul className="mt-1 list-disc ps-5">
              {contrastIssues.map((issue) => (
                <li
                  key={`${issue.mode}-${issue.pair}`}
                >{`${issue.mode}: ${issue.pair} ${issue.ratio.toFixed(2)}:1 < ${issue.required}:1`}</li>
              ))}
            </ul>
          </div>
        ) : (
          <p className="mt-3 text-success text-xs">
            {t("settings.theme.contrastPass")}
          </p>
        )}
      </Field>

      <Field
        hint={t("settings.theme.previewHint")}
        label={t("settings.theme.preview")}
      >
        <div className="mb-3 flex flex-wrap gap-2">
          ...
          >
            {previewArabic
              ? translateFn(
                  "settings.theme.preview.switchEnglish",
                  undefined,
                  "en",
                )
              : translateFn(
                  "settings.theme.preview.switchArabic",
                  undefined,
                  "ar",
                )}
          </button>
      ...
      <div className="mb-8 flex flex-wrap items-center gap-2 border-border border-y py-4">
        ...
        <button
          ...
          onClick={() =>
            change({
              theme: fullPresetTheme(resolved.id),
              appearance: draft.appearance,
            })
          }
          type="button"
      ...
      <Field
        hint={t("settings.theme.exchangeHint")}
        label={t("settings.theme.exchange")}
      >
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl border border-border p-4">
            <FileJson className="size-5 text-primary" />
            <h3 className="mt-3 font-semibold">
              {t("settings.theme.exportTitle")}
            </h3>
            <p className="mt-1 text-muted-foreground text-xs">
              {t("settings.theme.exportHint")}
            </p>
            <button
          ...
          <div className="rounded-xl border border-border p-4">
            ...
            <h3 className="mt-3 font-semibold">
              {t("settings.theme.importTitle")}
            </h3>
            <p className="mt-1 text-muted-foreground text-xs">
              {t("settings.theme.importHint")}
            </p>
            <input
        ...
        {importPreview ? (
          <div className="mt-4 rounded-xl border border-border bg-muted/25 p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <strong>
                {t("settings.theme.importChanges", {
                  count: importPreview.changes.length,
                })}
              </strong>
            ...
            <button
              ...
              disabled={
                !importTemplate ||
                importPreview.changes.length === 0 ||
                importProjectTheme.isPending
              }
              onClick={() =>
                importTemplate && importTheme(importTemplate, true)
              }
              type="button"
            >
