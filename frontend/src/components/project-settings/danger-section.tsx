...
import {
  useConfirm,
  usePrompt,
} from "@cms/design-system/components/ui/confirm";
import {
  Select,
  SelectContent,
  SelectItem,
      ...
      <div className="mb-3.5 flex flex-col items-stretch gap-4 rounded-2xl border border-destructive/30 p-5 sm:flex-row sm:items-center sm:gap-3.5">
        <p className="flex-1 text-[13.5px] text-muted-foreground leading-relaxed">
          <strong className="text-foreground">
            {t("settings.danger.transfer.title")}
          </strong>
          <br />
        ...
        <div className="flex w-full items-center gap-2 sm:min-w-[250px] sm:w-auto">
          ...
          >
            <SelectTrigger className="min-w-0 flex-1">
              <SelectValue
                placeholder={t("settings.danger.transfer.placeholder")}
              />
            </SelectTrigger>
            <SelectContent>
          ...
          <Button
            disabled={
              !canTransferOwnership || !targetMemberId || transfer.isPending
            }
            onClick={async () => {
              if (!selectedTarget) {
                return;
              ...
              const ok = await confirm({
                ...
                description: t("settings.danger.transfer.confirm", {
                  name: selectedTarget.user.name,
                }),
              ...
              transfer.mutate(
                { memberId: selectedTarget.id },
                {
                  onSuccess: () => {
                    toast.success(
                      t("settings.danger.transfer.toast.transferred"),
                    );
                    setTargetMemberId("");
                  },
                  onError: (error) =>
                    toast.error(
                      error instanceof Error
                        ? error.message
                        : t("settings.danger.transfer.toast.error"),
                    ),
                },
              );
            }}
        ...
        <Button
          ...
          onClick={async () => {
            ...
            const typed = await prompt({
              ...
              description: t("settings.danger.delete.confirm", {
                name: project.name,
              }),
              label: t("settings.danger.delete.typeToConfirm", {
                name: project.name,
              }),
            ...
            del.mutate(project.id, {
              ...
              onError: (error) =>
                toast.error(
                  error instanceof Error
                    ? error.message
                    : t("settings.danger.delete.toast.error"),
                ),
            });
          }}
          variant="destructive"
