...
export function DomainSection({ project }: { project: Project }) {
  ...
  const copyRecord = async (key: string, text: string) => {
    ...
    window.setTimeout(
      () => setCopiedRecord((current) => (current === key ? null : current)),
      1600,
    );
  };
              ...
              onError: (err) =>
                toast.error(
                  err instanceof Error
                    ? err.message
                    : t("settings.domain.toast.addError"),
                ),
            },
          );
        }}
      ...
      <div className="mt-4 space-y-3">
        {list.map((d) => (
          ...
          >
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  {d.dnsStatus === "VERIFIED" && d.sslStatus === "ACTIVE" ? (
                    <StatusBadge
                      label={t("settings.domain.status.verified")}
                      tone="ready"
                    />
                  ) : (
                    <StatusBadge
                ...
                {d.lastCheckedAt ? (
                  <p className="mt-1.5 text-muted-foreground text-xs">
                    {t("settings.domain.lastChecked", {
                      date: new Date(d.lastCheckedAt).toLocaleString(),
                    })}
                  </p>
                ) : null}
              </div>
                        ...
                        onSuccess: (result) =>
                          toast.success(
                            result.sslStatus === "ACTIVE"
                              ? t("settings.domain.toast.live")
                              : t("settings.domain.toast.provisioning"),
                          ),
                        onError: (error) =>
                          toast.error(
                            error instanceof Error
                              ? error.message
                              : t("settings.domain.toast.verifyError"),
                          ),
                      })
                    }
                    size="sm"
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setPrimary.mutate(d.id, {
                        onSuccess: () =>
                          toast.success(t("settings.domain.toast.primary")),
                      })
                    }
                    size="sm"
                    variant="outline"
            ...
            <CollapsibleContent>
              {d.lastError ? (
                <div className="mt-3 rounded-lg border border-destructive/25 bg-destructive/5 px-3 py-2 text-destructive text-sm">
                  {d.lastError}
                </div>
              ) : null}
                ...
                <div className="mt-4">
                  ...
                  <div className="overflow-hidden rounded-xl border border-border font-mono text-[12.5px]">
                    ...
                    {d.records.map((record) => {
                      ...
                      return (
                        <div
                          className="grid grid-cols-[72px_minmax(0,1fr)_minmax(0,1.35fr)_36px] items-center gap-2 px-3.5 py-2.5"
                          key={key}
                        >
                  ...
                  <p className="mt-3 text-[13px] text-muted-foreground">
                    {d.dnsStatus === "VERIFIED"
                      ? t("settings.domain.dns.configured")
                      : t("settings.domain.dns.propagation")}
                  </p>
                </div>
              ) : null}
            </CollapsibleContent>
