...
import { validateRedirectGraph } from "@cms/shared/redirects";
...
export function RedirectsSection({ project }: { project: Project }) {
  ...
  const form = useForm({
    defaultValues: {
      redirects: (project.config?.redirects ?? []).map((pair) => ({
        from: pair.from,
        to: pair.to,
      })),
    },
    onSubmit: async ({ value }) => {
      ...
      await saveConfigSection(update, {
        redirects: value.redirects.filter(
          (pair) => pair.from.trim() && pair.to.trim(),
        ),
      });
    },
  });
  ...
  return (
    ...
    >
      <SectionHeader
        ...
        title={t("settings.redirects.title")}
      />
          ...
          <form.Subscribe selector={(state) => state.values.redirects}>
            {(redirects) => {
              ...
              const invalidRows = new Set(
                issues.flatMap((issue) => issue.rowIndexes),
              );
              return (
                <>
                  {redirects.length > 0 ? (
                  ...
                  {issues.length > 0 ? (
                    <div
                      className="mb-4 rounded-xl border border-destructive/30 bg-destructive/5 p-3"
                      role="alert"
                    >
                      <div className="flex items-center gap-2 font-medium text-destructive text-xs">
                        <TriangleAlert className="size-3.5" />{" "}
                        {t("settings.redirects.invalid")}
                      </div>
      ...
      <form.Subscribe
        selector={(state) =>
          [state.isSubmitting, state.values.redirects] as const
        }
      >
        {([isSubmitting, redirects]) => (
          <SaveBar
            isSubmitting={isSubmitting}
            disabled={validateRedirectGraph(redirects).issues.length > 0}
          />
        )}
      </form.Subscribe>
    </form>
