...
import {
  useLanguages,
  useProjectSearchConfiguration,
  useUpdateLanguage,
  useUpdateProjectSearchConfiguration,
} from "@/hooks/api";
...
function ProjectSearchForm({
  ...
  if (configuration.isError || !configuration.data) {
    return (
      <SearchConfigurationError onRetry={() => void configuration.refetch()} />
    );
  }
  return (
    <>
  ...
  return (
    ...
    >
      <p className="font-medium text-destructive">
        {t("settings.search.configuration.error")}
      </p>
      <button
...
function ProjectSearchConfigurationForm({
  ...
  const [aiAnswers, setAiAnswers] = useState(
    configuration.aiAnswers ? "enabled" : "disabled",
  );
  const [filtersEnabled, setFiltersEnabled] = useState(
    configuration.filtersEnabled,
  );
  const [versionFilterEnabled, setVersionFilterEnabled] = useState(
    configuration.versionFilterEnabled,
  );
      ...
      try {
        await update.mutateAsync({
          ...
          maxResults: Number.isFinite(parsedMaxResults)
            ? Math.min(
                maxResultsConstraint.max,
                Math.max(maxResultsConstraint.min, parsedMaxResults),
              )
  ...
  return (
    ...
    >
      <form.Field name="placeholder">
        {(field) => (
          <Field
            hint={t("settings.search.placeholder.hint")}
            label={t("settings.search.placeholder.label")}
          >
            <Input
      ...
      <form.Field name="maxResults">
        {(field) => (
          <Field
            hint={t("settings.search.maxResults.hint")}
            label={t("settings.search.maxResults.label")}
          >
            <Input
              className={FIELD_INPUT}
      ...
      <Field
        hint={t("settings.search.hotkey.hint")}
        label={t("settings.search.hotkey.label")}
      >
        <Segmented
      ...
      <Field
        hint={t("settings.search.aiAnswers.hint")}
        label={t("settings.search.aiAnswers.label")}
      >
        <Segmented
          ...
          options={[
            ...
            {
              value: "disabled",
              label: t("settings.search.aiAnswers.disabled"),
            },
          ]}
          value={aiAnswers}
        />
    ...
    >
          ...
          >
            <Input
              ...
              placeholder={
                ...
                t("settings.search.placeholder.input")
              }
              value={field.state.value}
            />
          </Field>
      ...
      >
        <Input
          ...
          disabled
          type="number"
          value={String(projectSearch.maxResults ?? 12)}
        />
      </Field>
      <Field
        hint={t("settings.chrome.scope.globalField")}
