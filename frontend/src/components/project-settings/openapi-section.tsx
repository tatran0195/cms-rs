...
export function OpenApiSection({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    setSourceValue(
      current.source.type === "url"
        ? current.source.url
        : current.source.type === "repository"
          ? current.source.path
          : "",
    );
  }, [current]);
  ...
  const changeSource = (next: SourceType) => {
    ...
    if (current?.source.type === next) {
      ...
      if (current.source.type === "repository")
        existingValue = current.source.path;
    }
  ...
  const deleteReference = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("settings.openapi.deleteError"),
      );
    }
  };
  ...
  return (
    <div>
      <SectionHeader
        ...
        title={t("settings.openapi.title")}
        description={t("settings.openapi.description")}
      />
      ...
      {current && !editing ? (
        <Alert className="mb-6">
          ...
          <AlertTitle className="flex items-center gap-2">
            {t("settings.openapi.configured")}{" "}
            <Badge variant="secondary">{current.source.type}</Badge>
          </AlertTitle>
          <AlertDescription>
            <p>
              {t("settings.openapi.publishHint")} · /{current.path} ·{" "}
      ...
      {!current && !editing ? (
        <div className="rounded-xl border border-dashed p-6 text-center">
          ...
          <h3 className="mt-3 font-semibold">
            {t("settings.openapi.emptyTitle")}
          </h3>
          <p className="mx-auto mt-1 max-w-lg text-muted-foreground text-sm">
            {t("settings.openapi.emptyDescription")}
          </p>
      ...
      {editing ? (
        <>
          <Field
            label={t("settings.openapi.name")}
            hint={t("settings.openapi.nameHint")}
            htmlFor="openapi-title"
          >
            <Input
              id="openapi-title"
          ...
          <Field
            label={t("settings.openapi.path")}
            hint={t("settings.openapi.pathHint")}
            htmlFor="openapi-path"
          >
            <Input
              id="openapi-path"
          ...
          <Field
            label={t("settings.openapi.source")}
            hint={t("settings.openapi.sourceHint")}
          >
            <Segmented
              value={sourceType}
              ...
              options={[
                ...
                {
                  value: "repository",
                  label: t("settings.openapi.source.repository"),
                },
              ]}
            />
          </Field>
          ...
          {sourceType === "upload" ? (
            <div className="mb-6 rounded-lg border border-dashed p-4">
              ...
              >
                ...
                {filename || t("settings.openapi.chooseFile")}
              </label>
              <Input
                id="openapi-file"
                className="mt-3"
            ...
            <Field
              label={
                sourceType === "url"
                  ? t("settings.openapi.url")
                  : t("settings.openapi.repositoryPath")
              }
              hint={
                sourceType === "url"
                  ? t("settings.openapi.urlHint")
                  : t("settings.openapi.repositoryHint")
              }
            >
              <Input
                className={FIELD_MONO}
          ...
          <Alert className="mb-6">
            ...
            <AlertDescription>
              {t("settings.openapi.securityHint")}
            </AlertDescription>
          </Alert>
