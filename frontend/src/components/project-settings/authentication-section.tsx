...
import { jwtAccessConfigBody } from "@cms/validators";
...
export function AuthenticationSection({ project }: { project: Project }) {
  ...
  useEffect(() => {
    ...
    setPublicJwks(
      data.jwt.publicJwks ? JSON.stringify(data.jwt.publicJwks, null, 2) : "",
    );
  ...
  const invite = async () => {
    ...
    if (navigator.clipboard)
      await navigator.clipboard
        .writeText(result.activationUrl)
        .catch(() => undefined);
  ...
  const saveJwt = async () => {
    ...
    await updateJwt.mutateAsync(
      jwtAccessConfigBody.parse({
        ...
        ...(jwksUrl.trim()
          ? { jwksUrl: jwksUrl.trim(), publicJwks: null }
          : { jwksUrl: null, publicJwks: keys }),
        subjectClaim: "sub",
        emailClaim: "email",
        nameClaim: "name",
        groupsClaim,
  ...
  return (
    <div>
      ...
      />
      <Field
        hint={t("settings.authentication.reader.modeHint")}
        label={t("settings.authentication.reader.mode")}
      >
        <Segmented
          ...
          options={[
            {
              value: "PUBLIC",
              label: t("settings.authentication.mode.public"),
            },
            {
              value: "WORKSPACE",
              label: t("settings.authentication.reader.workspace"),
            },
            {
              value: "READERS",
              label: t("settings.authentication.reader.private"),
            },
          ]}
          value={mode}
        />
        <Button
          ...
          onClick={() =>
            void saveMode().catch((error) => toast.error(error.message))
          }
          type="button"
        >
      ...
      {data?.accessMode === "READERS" ? (
        <>
          <Panel
            title={t("settings.authentication.reader.audiencesTitle")}
            description={t(
          ...
          >
            ...
            <p className="mt-2 text-muted-foreground text-xs">
              {t("settings.authentication.reader.audienceAllPages")}
            </p>
            <Button
              className="mt-3"
              disabled={!audienceName.trim()}
              onClick={() =>
                void addAudience().catch((error) => toast.error(error.message))
              }
              type="button"
            >
                  ...
                  <div>
                    ...
                    <div className="text-muted-foreground text-xs">
                      {audience.grants.some((grant) => !grant.pageId)
                        ...
                        : t("settings.authentication.reader.pageCount", {
                            count: audience.grants.length,
                          })}{" "}
                      ·{" "}
                      {t("settings.authentication.reader.readerCount", {
                        count: audience._count.readers,
                      })}
                    </div>
                  </div>
                  <Button
          ...
          <Panel
            title={t("settings.authentication.reader.readersTitle")}
            description={t("settings.authentication.reader.readersDescription")}
          >
            ...
            <Button
              ...
              onClick={() =>
                void invite().catch((error) => toast.error(error.message))
              }
              type="button"
            >
                ...
                >
                  <div>
                    <div className="font-medium">
                      {reader.name ||
                        reader.email ||
                        t("settings.authentication.reader.portalReader")}
                    </div>
                    <div className="text-muted-foreground text-xs">
                      {reader.status} ·{" "}
                      {reader.audiences
                        .map((item) => item.audience.name)
                        .join(", ")}{" "}
                      ·{" "}
                      {t("settings.authentication.reader.sessionCount", {
                        count: reader._count.sessions,
                      })}
                    </div>
                  </div>
                  <Button
                      ...
                      window.confirm(
                        t("settings.authentication.reader.revokeConfirm", {
                          name:
                            reader.name ||
                            reader.email ||
                            t("settings.authentication.reader.thisReader"),
                        }),
                      )
                        ? void revokeReader
                            .mutateAsync(reader.id)
                            .catch((error) => toast.error(error.message))
                        : undefined
                    }
                    size="sm"
          ...
          <Panel
            title={t("settings.authentication.reader.jwtTitle")}
            description={t("settings.authentication.reader.jwtDescription")}
          >
            <Button
              onClick={() => setShowJwt((value) => !value)}
              type="button"
              variant="outline"
            >
              {showJwt
            ...
            {showJwt ? (
              <div className="mt-4">
                <label className="mb-3 flex items-center gap-2 text-sm">
                  <input
                    checked={jwtEnabled}
                    onChange={(event) => setJwtEnabled(event.target.checked)}
                    type="checkbox"
                  />{" "}
                ...
                <Button
                  className="mt-3"
                  onClick={() =>
                    void saveJwt().catch((error) => toast.error(error.message))
                  }
                  type="button"
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void testJwt
                        ...
                        .then((result) =>
                          toast.success(
                            `${t("settings.authentication.reader.jwtValid")}: ${JSON.stringify(result)}`,
                          ),
                        )
                        .catch((error) => toast.error(error.message))
                    }
          ...
          <Panel
            title={t("settings.authentication.reader.auditTitle")}
            description={t("settings.authentication.reader.auditDescription")}
          >
            <Button
            ...
            >
              {showAudit
                ? t("settings.authentication.reader.auditHide")
                : t("settings.authentication.reader.auditReview")}
            </Button>
            {showAudit ? (
              <div className="mt-4">
                <Button
                  onClick={() =>
                    window.confirm(
                      t("settings.authentication.reader.emergencyConfirm"),
                    )
        ...
        <div className="mt-6 rounded-xl border border-dashed p-6 text-center">
          <h3 className="font-semibold">
            {t("settings.authentication.reader.inactiveTitle")}
          </h3>
          <p className="mx-auto mt-1 max-w-xl text-muted-foreground text-sm">
            {t("settings.authentication.reader.inactiveDescription")}
          </p>
        </div>
      )}
    </div>
