...
const PRESETS = {
  ...
  string,
  { baseSize: BaseSize; leading: Leading; flow: Flow }
>;
...
function RhythmPreview({
  ...
  return (
    <div
      className="typeset max-h-72 overflow-y-auto rounded-xl border border-border bg-background p-5"
      style={style}
    >
      ...
      <ul>
        ...
        <li>
          {t("settings.typography.preview.item2")
            .split("`")
            .map((part, i) =>
              i % 2 === 1 ? <code key={part}>{part}</code> : part,
            )}
        </li>
      </ul>
    </div>
...
export function TypographySection({ project }: { project: Project }) {
  ...
  const [baseSize, setBaseSize] = useState<BaseSize>(
    (typography.baseSize as BaseSize) ?? "16",
  );
  const [leading, setLeading] = useState<Leading>(
    (typography.leading as Leading) ?? "1.75",
  );
  ...
  const activePreset = (Object.keys(PRESETS) as PresetName[]).find(
    (name) =>
      PRESETS[name].baseSize === baseSize &&
      PRESETS[name].leading === leading &&
      PRESETS[name].flow === flow,
  );
  ...
  return (
    ...
    >
      ...
      <Field
        hint={t("settings.typography.preset.hint")}
        label={t("settings.typography.preset.label")}
      >
      ...
      <form.Field name="headingFont">
        {(field) => (
          <Field
            hint={t("settings.typography.headingFont.hint")}
      ...
      <form.Field name="bodyFont">
        {(field) => (
          <Field
            hint={t("settings.typography.bodyFont.hint")}
      ...
      <form.Field name="codeFont">
        {(field) => (
          <Field
            hint={t("settings.typography.codeFont.hint")}
      ...
      <Field
        hint={t("settings.typography.baseSize.hint")}
        label={t("settings.typography.baseSize.label")}
      >
        <Segmented
      ...
      <Field
        hint={t("settings.typography.leading.hint")}
        label={t("settings.typography.leading.label")}
      >
        <Segmented
      ...
      <Field
        hint={t("settings.typography.flow.hint")}
        label={t("settings.typography.flow.label")}
      >
        <Segmented
      ...
      <form.Subscribe
        selector={(state) =>
          [
            state.values.headingFont,
            state.values.bodyFont,
            state.values.codeFont,
          ] as const
        }
      >
        {([headingFont, bodyFont, codeFont]) => (
          <Field
            hint={t("settings.typography.preview.hint")}
            label={t("settings.typography.preview.label")}
          >
            <RhythmPreview
              baseSize={baseSize}
              bodyFont={bodyFont}
