...
describe("published theme projection", () => {
  ...
  it("does not interpolate an unsafe font into CSS", () => {
    const css = projectThemeCss({
      typography: { headingFont: "Inter';}body{display:none}/*" },
    });
  ...
  it("renders an RTL theme preview while preserving LTR code", () => {
    const markup = renderToStaticMarkup(
      <ThemePreview
        arabic
        config={{ theme: { preset: "manuscript" } }}
        mode="dark"
      />,
    );
    ...
    expect(markup).toContain(
      translateFn("settings.theme.preview.productDocs", undefined, "ar"),
    );
  });
  ...
  it.each([
    ...
    "renders the %s preset with its own reader and page templates",
    (preset, shell, layout) => {
      ...
      const markup = renderToStaticMarkup(
        ...
        >
          ...
          <DocumentationPageLayout
            ...
            tableOfContents={<nav>outline</nav>}
          />
          <DocumentationProjectPreviewLayout
  ...
  it("selects templates by the customizable shell contract without changing preset metadata", () => {
    const theme = resolveProjectTheme({
      theme: { preset: "harbor", layout: { shell: "console" } },
    });
    const markup = renderToStaticMarkup(
      <DocumentationThemeProvider
        context="reader"
    ...
    expect(Object.keys(DOCUMENTATION_THEME_TEMPLATES)).toEqual([
      "reference",
      "editorial",
      "console",
    ]);
