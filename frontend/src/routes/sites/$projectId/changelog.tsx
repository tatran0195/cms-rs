...
const formatDate = (value: string | null, lang?: string): string => {
  ...
  return date
    ? date.toLocaleDateString(lang || undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
...
function groupByMonth(
  ...
  lang?: string,
): Array<{ key: string; label: string; entries: ChangelogEntry[] }> {
  const groups: Array<{
    key: string;
    label: string;
    entries: ChangelogEntry[];
  ...
  for (const entry of entries) {
    ...
    if (last && last.key === key) {
      ...
      groups.push({
        key,
        label: monthLabel(entry.date, lang),
        entries: [entry],
      });
    }
  }
          ...
          <div className="space-y-10">
            {groups.map((group) => (
              <section key={group.key}>
                <h2 className="mb-4 font-semibold text-[11px] text-muted-foreground/80 uppercase tracking-wider">
                  {group.label}
                </h2>
