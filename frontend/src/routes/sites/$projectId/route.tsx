...
} from "@/components/site/page-alternates-context";
...
export const Route = createFileRoute("/sites/$projectId")({
  ...
  loader: async ({ params, deps, location }) => {
    try {
      const rest = location.pathname
        .replace(new RegExp(`^/sites/${params.projectId}/?`), "")
        .replace(/\/+$/, "");
      const candidate =
        rest && rest !== "changelog"
          ? decodeURIComponent(rest).split("/")[0]
          : undefined;
  ...
  head: ({ loaderData }) =>
    siteHead(loaderData?.site ?? null, loaderData?.siteOrigin),
});
...
function SiteChrome() {
  ...
  const currentPath = decodeURIComponent(
    pathname.replace(new RegExp(`^/sites/${projectId}/?`), ""),
  ).replace(/\/+$/, "");
  const isChangelog = currentPath === "changelog";
  const currentVersion =
    site?.versions.find((item) => item.slug === site.activeVersion) ??
    site?.versions.find((item) => item.isDefault);
  const activeVersionPrefix =
    currentVersion && !currentVersion.isDefault
      ? currentVersion.slug
      : undefined;
  const contentPath =
    activeVersionPrefix &&
    (currentPath === activeVersionPrefix ||
      currentPath.startsWith(`${activeVersionPrefix}/`))
  ...
  const effectiveCurrentPath =
    contentPath || firstLeafPath(site?.nav ?? []) || "";
  ...
  useEffect(() => {
    ...
    const dashboardUsesSystem =
      dashboardTheme !== "light" && dashboardTheme !== "dark";
  ...
  const pageAlternatesContext = useMemo(
    () => ({ alternates: pageAlternates, setAlternates: setPageAlternates }),
    [pageAlternates],
  );
  ...
  // "Made with CMS" attribution is on by default.
  const showBadge = footer?.madeWithBadge !== false;
  const hasFooterContent = Boolean(
    footer &&
    (footer.copyright || footer.github || footer.x || footer.linkedin),
  );
      ...
      {logo ? (
        <>
          <img
            ...
            className={cn(
              "object-contain",
              logo.markOnly ? "size-7" : "h-6 w-auto",
            )}
          />
          {logo.markOnly ? null : (
  ...
  const changeLanguage = (code: string) => {
    const alternate = pageAlternates.find(
      (item) => item.code === code && item.path,
    );
  ...
  const changeVersion = (slug: string) => {
    ...
    const targetPath = [targetPrefix, isChangelog ? "" : contentPath]
      .filter(Boolean)
      .join("/");
    window.location.assign(siteHref(projectId, targetPath, { lang }));
  };
  const activeVersion =
    site?.activeVersion ?? versions.find((item) => item.isDefault)?.slug ?? "";
  const sitePath = (path = "") =>
    siteHref(projectId, path, { lang, version: activeVersionPrefix });
  ...
  const resolveNavHref = (href: string): string =>
    href.startsWith("/") && !href.startsWith("//")
      ? siteHref(projectId, href, { lang, version: activeVersionPrefix })
      : href;
  const isNavActive = (href: string): boolean => {
    ...
    return (
      prefix !== "" &&
      (effectiveCurrentPath === prefix ||
        effectiveCurrentPath.startsWith(`${prefix}/`))
    );
  };
            ...
            >
              ...
              >
                  ...
                  {showSearch ? (
                    ...
                    >
                      ...
                      <span className="truncate">
                        {config?.search?.placeholder ?? t("search")}
                      </span>
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <LanguageSwitcher
                    languages={languages}
                    activeCode={activeLanguage?.code ?? ""}
                    onChange={changeLanguage}
                  />
                  <VersionSwitcher
                    versions={versions}
                    activeSlug={activeVersion}
              ...
              {navTabs.length > 0 ? (
                <nav
                  className="mx-auto flex h-11 max-w-[90rem] items-center gap-1 overflow-x-auto px-4 sm:px-6"
                  data-theme-region="tabs"
                >
                  {navTabs.map((tab) => {
                    ...
                    <li key={`${anchor.label}-${anchor.href}`}>
                      ...
                      >
                        <span className="grid size-6 shrink-0 place-items-center rounded-md border border-border bg-card shadow-2xs transition-colors group-hover:border-primary/40 group-hover:text-primary">
                          {hasIcon(anchor.icon) ? (
                            <PageIcon name={anchor.icon} className="size-3.5" />
                          ) : (
              ...
              <footer className="mt-auto border-border/60 border-t">
                {hasFooterContent && footer ? (
                  ...
                  >
                    <span>
                      {footer.copyright ??
                        `© ${new Date().getFullYear()} ${siteName ?? ""}`.trim()}
                    </span>
                ...
                {showBadge ? (
                  <div
                    className={cn(
                      "mx-auto max-w-[90rem] px-6",
                      hasFooterContent ? "pb-6" : "py-5",
                    )}
                  >
              ...
              {showSearch ? (
                <SiteSearch
                  ...
                  versionFilterEnabled={
                    config?.search?.versionFilterEnabled !== false
                  }
