...
function WorkspaceAnalyticsPage() {
  ...
  const { data, isPending, isError } = useWorkspaceAnalytics(range, {
    timezone,
  });
  ...
  const emptySearches = unavailable
    ? t("analytics.state.unknown")
    : t("analytics.empty.searches");
  ...
  const projectsWithTraffic = unavailable
    ? null
    : byProject.filter((p) => p.views > 0).length;
  const hasTimeseries = (data?.timeseries ?? []).some((d) => d.views > 0);
  const zeroResults =
    data?.searches && "zeroResults" in data.searches
      ? data.searches.zeroResults
      : null;
  const clickedResults =
    data?.searches && "clickedResults" in data.searches
      ? data.searches.clickedResults
      : null;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="mb-4 font-medium text-sm">
          {t("analytics.chart.pageviewsAllProjects")}
        </div>
        {isPending ? (
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          value={zeroResults ?? null}
          icon={<Search className="size-4" />}
          loading={isPending}
        />
        <StatCard
          label={t("analytics.kpi.resultClicks")}
          value={clickedResults ?? null}
          icon={<Search className="size-4" />}
          loading={isPending}
        />
        <StatCard
