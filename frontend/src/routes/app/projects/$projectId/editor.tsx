...
function EditorPage() {
  ...
  const synced = useRef<{ id: string; content: string; title: string } | null>(
    null,
  );
  const hydrating = useRef<{
    id: string;
    content: string;
    title: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const clean = synced.current
      ? content === synced.current.content && title === synced.current.title
      : true;
  ...
  const openDraftPreview = async () => {
    ...
    try {
      const draftIsCurrent =
        page &&
        synced.current?.id === page.id &&
        synced.current.title === title &&
        synced.current.content === content;
          ...
          <section className="flex min-h-0 min-w-0 flex-col">
            ...
            >
              ...
              <div
                className={cn(
                  effectiveMode === "markdown"
                    ? "w-full shrink-0 border-border border-b px-6 py-4"
                    : "mx-auto max-w-[720px]",
                )}
              >
                <input
