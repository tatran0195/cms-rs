...
function AnalyticsPage() {
  ...
  const { data, isPending, isError } = useProjectAnalytics(projectId, range, {
    timezone,
  });
  const unavailable = isError || data?.availability === "unavailable";
  const partial = data?.availability === "partial";
  const unknownOr = (empty: string) =>
    unavailable ? t("analytics.state.unknown") : empty;
    ...
    <div className="w-full px-6 py-8 xl:px-10">
      ...
      {unavailable ? (
        <Alert className="mt-6" variant="warning">
          ...
          <AlertDescription>
            {t("analytics.state.unavailable.body")}
          </AlertDescription>
        </Alert>
      ) : null}
      {partial ? (
        <Alert className="mt-6" variant="info">
          ...
          <AlertDescription>
            {t("analytics.state.partial.body")}
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        />
        <StatCard
          label={t("analytics.kpi.searches")}
          value={data?.searches.total ?? null}
          loading={isPending}
        />
        <StatCard
      ...
      <div className="mt-4 rounded-xl border border-border bg-card p-5">
        <div className="mb-4 font-medium text-sm">
          {t("analytics.chart.pageviewsOverTime")}
        </div>
        {isPending ? (
          <Skeleton className="h-64 w-full" />
        ) : unavailable ? (
          <p className="py-20 text-center text-muted-foreground text-sm">
            {t("analytics.state.unknown")}
          </p>
        ) : !hasTimeseries ? (
          <p className="py-20 text-center text-muted-foreground text-sm">
            {t("analytics.empty.pageviews")}
          </p>
        ) : (
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        {data?.searches.queryTerms === "legacy" ? (
          ...
          <ListCard
            ...
            empty={unknownOr(t("analytics.empty.searches"))}
            items={[
              ...(data?.searches.clickedResults === null ||
              data?.searches.clickedResults === undefined
                ? []
                : [
                    {
                      label: t("analytics.kpi.resultClicks"),
                      value: data.searches.clickedResults,
                    },
                  ]),
              ...(data?.searches.averageLatencyMs === null ||
              data?.searches.averageLatencyMs === undefined
                ? []
                : [
                    {
                      label: t("analytics.kpi.averageLatencyMs"),
                      value: data.searches.averageLatencyMs,
                    },
                  ]),
            ]}
          />
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <ListCard
          ...
          items={[
            ...(data?.ai.answersCompleted === null ||
            data?.ai.answersCompleted === undefined
              ? []
              : [
                  {
                    label: t("analytics.kpi.answersCompleted"),
                    value: data.ai.answersCompleted,
                  },
                ]),
            ...(data?.ai.answersFailed === null ||
            data?.ai.answersFailed === undefined
              ? []
              : [
                  {
                    label: t("analytics.kpi.answersFailed"),
                    value: data.ai.answersFailed,
                  },
                ]),
            ...(data?.ai.costMicros === null ||
            data?.ai.costMicros === undefined
              ? []
              : [
                  {
                    label: t("analytics.kpi.aiCostMicros"),
                    value: data.ai.costMicros,
                  },
                ]),
          ]}
        />
      ...
      <p className="mt-4 text-muted-foreground text-xs">
        {t("analytics.privacy.queryTerms")}
      </p>
    </div>
  );
}
