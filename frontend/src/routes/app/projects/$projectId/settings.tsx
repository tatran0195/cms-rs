...
export const Route = createFileRoute("/app/projects/$projectId/settings")({
  ...
  validateSearch: (
    ...
    section: isSectionId(search.section)
      ? search.section
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col overflow-hidden bg-background md:flex-row">
      <div className="shrink-0 border-border border-b bg-card px-4 py-3 md:hidden">
        <label
          className="mb-1.5 block font-semibold text-[10.5px] text-muted-foreground uppercase tracking-wider"
        ...
        <select
          ...
          onChange={(event) =>
            navigate({
              search: { section: event.target.value as SectionId },
              replace: true,
            })
          }
          value={section}
        >
          {GROUPS.map((group) => (
            <optgroup key={group.id} label={t(group.labelKey)}>
              {SECTIONS.filter((item) => item.group === group.id).map(
                (item) => (
      ...
      <aside className="hidden w-[238px] shrink-0 overflow-y-auto border-border border-e bg-card px-3 py-4.5 md:block">
        <div className="px-3 pt-1 pb-2.5 font-bold text-[11px] text-muted-foreground uppercase tracking-wider">
          {t("settings.heading")}
        </div>
        <nav className="flex flex-col gap-0.5">
          {GROUPS.map((group) => (
            <div key={group.id} className="mt-3 first:mt-0">
              <div className="px-3 pb-1 font-semibold text-[10.5px] text-muted-foreground/70 uppercase tracking-wider">
                {t(group.labelKey)}
              </div>
              {SECTIONS.filter((item) => item.group === group.id).map(
                (item) => {
...
function ActiveSection({
  ...
  const sections: Record<SectionId, ReactNode> = {
    ...
    languages: (
      <LanguagesSection key={`languages-${project.id}`} project={project} />
    ),
    domain: <DomainSection key={`domain-${projectId}`} project={project} />,
    authentication: (
      <AuthenticationSection
        key={`authentication-${project.id}`}
        project={project}
      />
    ),
    search: <SearchSection key={`search-${project.id}`} project={project} />,
    addons: (
      <AddonsSection key={`addons-${project.id}`} projectId={project.id} />
    ),
    git: <GitTab key={`git-${projectId}`} projectId={projectId} />,
    openapi: (
      <OpenApiSection key={`openapi-${projectId}`} projectId={projectId} />
    ),
    contentImport: (
      <ImportTab key={`import-${projectId}`} projectId={projectId} />
    ),
    members: (
      <MembersSection key={`members-${projectId}`} projectId={projectId} />
    ),
    ...
    exports: (
      <ExportsSection key={`exports-${projectId}`} projectId={projectId} />
    ),
