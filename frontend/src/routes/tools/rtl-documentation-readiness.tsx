...
import {
  ...
  RTL_RUBRIC_VERSION,
  type RtlReadinessResult,
} from "@/lib/rtl-readiness";
...
export const Route = createFileRoute("/tools/rtl-documentation-readiness")({
  ...
  head: () => ({
    meta: pageMeta({
      title: "Free RTL documentation readiness grader | CMS",
      description:
...
function RtlDocumentationReadinessPage() {
  ...
  const copyResult = async () => {
    ...
    try {
      ...
      setCopied(false);
      setCopyError(
        "Clipboard access was unavailable. Open the full check list and copy the result manually.",
      );
    }
  };
    ...
    <MarketingShell stars={stars}>
      <section className="border-border border-b">
        <div className="mx-auto max-w-4xl px-6 py-16 text-center sm:py-20">
          ...
          <h1 className="mt-4 text-balance font-semibold text-4xl tracking-tight sm:text-5xl">
            RTL documentation readiness grader
          </h1>
          <p className="mx-auto mt-4 max-w-3xl text-balance text-lg text-muted-foreground leading-relaxed">
            Paste a page's HTML to inspect Arabic language signals, bidi code
            handling, search controls, navigation, semantics, and
          ...
          <div className="mx-auto mt-6 flex max-w-2xl items-start gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/8 p-4 text-start text-sm">
            ...
            <p>
              <strong>Your HTML stays in this browser.</strong> The grader does
              not upload, store, or include submitted content or URLs in
              analytics.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              <h2 className="font-semibold text-2xl tracking-tight">
                Paste the initial HTML response
              </h2>
              <p className="mt-1 text-muted-foreground text-sm">
                Use View Source where possible. DOM copied after hydration may
                hide SSR defects.
              </p>
            </div>
      ...
      <section className="border-border border-y bg-card/40">
        <div className="mx-auto max-w-5xl px-6 py-16">
          <h2 className="font-semibold text-3xl tracking-tight">
            What this score can—and cannot—prove
          </h2>
          ...
          <p className="mt-6 text-muted-foreground text-sm leading-relaxed">
            Unknowns never become failures or zero. A complete review combines
            this static report with the open fixture corpus, browser checks, and
            real search queries.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <div className="rounded-2xl border border-border bg-foreground px-8 py-12 text-background sm:px-12">
          <h2 className="font-semibold text-3xl tracking-tight">
            Test the same workflow in a bilingual project
          </h2>
          <p className="mt-3 max-w-2xl text-background/75 leading-relaxed">
            Create one Arabic page, include a command and path, publish it
            manually, then compare the live HTML and search results with this
            report.
          </p>
  ...
  return (
    <div className="flex min-h-[34rem] flex-col items-center justify-center rounded-2xl border border-border border-dashed bg-card/30 p-8 text-center">
      ...
      <h2 className="mt-4 font-semibold text-xl">
        Your evidence report appears here
      </h2>
      <p className="mt-2 max-w-sm text-muted-foreground text-sm">
        The included sample demonstrates the input format. Replace it with a
        real initial HTML response when you are ready.
      </p>
    </div>
  );
}
        ...
        {result.categories.map((category) => (
          ...
          >
            ...
            <p className="mt-1 text-muted-foreground text-xs">
              {category.knownWeight}/{category.totalWeight} weight scored ·{" "}
              {category.unknownChecks} unknown
            </p>
          </div>
        ))}
      </div>
      <details
        className="mt-6 rounded-lg border border-border bg-background px-4"
        open
      >
        <summary className="cursor-pointer py-4 font-medium text-sm">
          All checks and reproduction steps
        </summary>
