...
export const Route = createFileRoute("/developers")({
  ...
  head: () => ({
    meta: pageMeta({
      title: "CMS developer resources: OpenAPI, agents, and CLI",
      description:
        "Discover the CMS public reader API, OpenAPI schema, Markdown content negotiation, llms files, authentication boundaries, and official CLI.",
      path: "/developers",
    }),
...
export function DeveloperResourcesPage({ stars = 0 }: { stars?: number }) {
  ...
  const resources = [
    ...
    },
    {
      ...
      href:
        locale === "ar"
          ? "https://docs.cms.com/ar/self-hosting/mcp"
          : "https://docs.cms.com/self-hosting/mcp",
      label: t("marketing.release.mcpLabel"),
    },
    {
      icon: Terminal,
      title: "Official CMS CLI",
      body: "The @cms/cli package inspects a CMS site’s machine-readable endpoints and fetches canonical pages as Markdown for scripts and agents.",
      href: "https://www.npmjs.com/package/@cms/cli",
  ...
  return (
    <MarketingShell stars={stars}>
      <section className="border-border border-b">
        <div className="mx-auto max-w-4xl px-6 py-20 text-center">
          ...
          <h1 className="mt-4 text-balance font-semibold text-4xl tracking-tight sm:text-5xl">
            CMS developer resources
          </h1>
          <p className="mx-auto mt-4 max-w-3xl text-balance text-lg text-muted-foreground leading-relaxed">
            Discover the supported public reader API, machine-readable product
            files, Markdown negotiation contract, and command-line tooling
            without guessing private dashboard routes.
          </p>
        </div>
      </section>
      ...
      <section
        ...
        aria-labelledby="developer-entry-points"
      >
        <h2
          className="font-semibold text-3xl tracking-tight"
          id="developer-entry-points"
        >
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-10 px-6 py-20 lg:grid-cols-2">
          <div>
            <h2 className="font-semibold text-2xl tracking-tight">
              Call CMS from an agent
            </h2>
            <ol className="mt-5 space-y-3 text-muted-foreground leading-relaxed">
              <li>
                1. Read /llms.txt to choose the right product or documentation
                resource.
              </li>
              <li>
                2. Send Accept: text/markdown to a canonical CMS marketing URL
                when clean page content is needed.
              </li>
              <li>
                3. Read /openapi.json before calling the public reader API;
                preserve typed parameters and operation IDs.
              </li>
              <li>
                4. Treat a 404 as a missing path and recover through the sitemap
                or llms index.
              </li>
            </ol>
          </div>
          <div>
            ...
            <pre
              className="mt-5 overflow-x-auto rounded-xl border border-border bg-background p-5 font-mono text-sm leading-7"
              dir="ltr"
            >
              <code>
                {
            ...
            <p className="mt-4 text-muted-foreground text-sm leading-relaxed">
              The CLI uses standard HTTP endpoints and does not collect
              credentials. Its inspect command checks status, content type,
              OpenAPI, llms, sitemap, Markdown negotiation, and real 404
              behavior.
            </p>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20">
        <div className="rounded-xl border border-border bg-card p-7 sm:p-9">
          ...
          <p className="mt-3 text-muted-foreground leading-relaxed">
            Public published sites require no credentials. Private reader sites
            use their configured reader session and intentionally return 404 to
            unauthorized callers. Dashboard endpoints use an authenticated
            browser session and are not a supported third-party write API. Do
            not send session cookies or secrets to the CLI.{" "}
            {t("marketing.release.mcpBoundary")}
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              className={primaryButton}
              href="https://docs.cms.com/reference/api"
            >
              Read CMS API documentation{" "}
              <ArrowRight aria-hidden="true" className="size-4" />
            </a>
            <a
              className="inline-flex h-10 items-center gap-2 px-2 font-medium text-primary text-sm hover:underline"
              href="/contact"
            >
