...
export const Route = createFileRoute("/git-preview/$token")({
  loader: async ({ params }) =>
    JSON.parse(await getGitPreviewFn({ data: { token: params.token } })) as {
      snapshot: SiteSnapshot;
    },
  head: () => ({
    meta: [
      { title: "Pull request preview · CMS" },
...
function GitPreviewPage() {
  ...
  const pages = useMemo(
    () => snapshot.pages.filter((page) => page.kind === "PAGE" && !page.hidden),
    [snapshot.pages],
  );
      ...
      <aside className="flex min-h-screen flex-col border-border border-e bg-card/60">
        <div className="border-border border-b p-4">
          ...
          <p className="mt-1 text-muted-foreground text-xs">
            Immutable snapshot · not indexed
          </p>
        </div>
            ...
            <>
              ...
              <Markdown
                ...
                site={{
                  ...
                  version: page.versionId,
                }}
              />
            </>
          ) : (
            <p className="text-muted-foreground">
              This preview contains no visible pages.
            </p>
          )}
        </article>
      </main>
