...
// src/table/utilities/keepCursorInTable.ts
var import_core5 = require("@tiptap/core");
var import_state = require("@tiptap/pm/state");
function keepCursorInTable(tr, tablePos) {
  const mappedTablePos = tr.mapping.map(tablePos);
  const stillInTable = (0, import_core5.findParentNodeClosestToPos)(
    tr.selection.$from,
  ...
  tr.setSelection(import_state.TextSelection.near(tr.doc.resolve(endOfTable), -1));
}
...
// src/table/table.ts
var Table = import_core6.Node.create({
  name: "table",
  // @ts-ignore
  addOptions() {
    return {
    ...
    return {
      insertTable: ({ rows = 3, cols = 3, withHeaderRow = true } = {}) => ({ tr, dispatch, editor }) => {
        ...
        if (dispatch) {
          ...
          tr.replaceSelectionWith(node).scrollIntoView().setSelection(import_state2.TextSelection.near(tr.doc.resolve(offset)));
        }
        return true;
      },
      ...
      deleteColumn: () => ({ state, dispatch }) => {
        const table = (0, import_core6.findParentNodeClosestToPos)(
          state.selection.$from,
          (node) => node.type.name === "table"
        );
        return (0, import_tables2.deleteColumn)(
          state,
          dispatch && ((tr) => {
            if (table) {
      ...
      deleteRow: () => ({ state, dispatch }) => {
        ...
        return (0, import_tables2.deleteRow)(
          state,
          dispatch && ((tr) => {
            if (table) {
              keepCursorInTable(tr, table.pos);
  ...
  addNodeView() {
    ...
    return ({ node, view, HTMLAttributes }) => {
      const mergedAttributes = (0, import_core6.mergeAttributes)(this.options.HTMLAttributes, HTMLAttributes);
  ...
  extendNodeSchema(extension) {
    ...
    return {
      tableRole: (0, import_core6.callOrReturn)((0, import_core6.getExtensionField)(extension, "tableRole", context))
    };
  }
});

// src/kit/index.ts
var TableKit = import_core7.Extension.create({
  name: "tableKit",
  addExtensions() {
    const extensions = [];
