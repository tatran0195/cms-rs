...
function useQueuer(fn, options = {}, selector = () => ({})) {
	...
	const [queuer] = (0, react.useState)(() => {
		...
		queuerInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(queuerInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(queuer);
			else queuer.stop();
		};
	}, []);
	const state = (0, _tanstack_react_store.useSelector)(queuer.store, selector, { compare: _tanstack_react_store.shallow });
