...
function useDebouncer(fn, options, selector = () => ({})) {
	...
	const [debouncer] = (0, react.useState)(() => {
		...
		debouncerInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(debouncerInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(debouncer);
			else debouncer.cancel();
		};
	}, []);
	const state = (0, _tanstack_react_store.useSelector)(debouncer.store, selector, { compare: _tanstack_react_store.shallow });
