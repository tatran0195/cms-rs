...
function useAsyncQueuer(fn, options = {}, selector = () => ({})) {
	...
	const [asyncQueuer] = (0, react.useState)(() => {
		...
		asyncQueuerInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(asyncQueuerInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(asyncQueuer);
			else {
				asyncQueuer.stop();
				asyncQueuer.abort();
			}
	...
	const state = (0, _tanstack_react_store.useSelector)(asyncQueuer.store, selector, { compare: _tanstack_react_store.shallow });
