...
function useAsyncRateLimiter(fn, options, selector = () => ({})) {
	...
	const [asyncRateLimiter] = (0, react.useState)(() => {
		...
		asyncRateLimiterInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(asyncRateLimiterInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(asyncRateLimiter);
			else asyncRateLimiter.abort();
		};
	}, []);
	const state = (0, _tanstack_react_store.useSelector)(asyncRateLimiter.store, selector, { compare: _tanstack_react_store.shallow });
