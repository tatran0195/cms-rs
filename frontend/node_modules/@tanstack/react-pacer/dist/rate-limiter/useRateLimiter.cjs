...
let _tanstack_pacer_rate_limiter = require("@tanstack/pacer/rate-limiter");
...
function useRateLimiter(fn, options, selector = () => ({})) {
	const mergedOptions = {
		...require_PacerProvider.useDefaultPacerOptions().rateLimiter,
		...options
	};
	const [rateLimiter] = (0, react.useState)(() => {
		const rateLimiterInstance = new _tanstack_pacer_rate_limiter.RateLimiter(fn, mergedOptions);
		rateLimiterInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(rateLimiterInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(rateLimiter);
		};
	}, []);
	const state = (0, _tanstack_react_store.useSelector)(rateLimiter.store, selector, { compare: _tanstack_react_store.shallow });
...
//# sourceMappingURL=useRateLimiter.cjs.map