...
let _tanstack_pacer_throttler = require("@tanstack/pacer/throttler");
...
function useThrottler(fn, options, selector = () => ({})) {
	const mergedOptions = {
		...require_PacerProvider.useDefaultPacerOptions().throttler,
		...options
	};
	const [throttler] = (0, react.useState)(() => {
		const throttlerInstance = new _tanstack_pacer_throttler.Throttler(fn, mergedOptions);
		throttlerInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(throttlerInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	const state = (0, _tanstack_react_store.useSelector)(throttler.store, selector, { compare: _tanstack_react_store.shallow });
	(0, react.useEffect)(() => {
		return () => {
...
//# sourceMappingURL=useThrottler.cjs.map