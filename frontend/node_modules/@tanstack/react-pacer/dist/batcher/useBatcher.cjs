...
function useBatcher(fn, options = {}, selector = () => ({})) {
	...
	const [batcher] = (0, react.useState)(() => {
		...
		batcherInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(batcherInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(batcher);
			else batcher.cancel();
		};
	}, []);
	const state = (0, _tanstack_react_store.useSelector)(batcher.store, selector, { compare: _tanstack_react_store.shallow });
