...
function useAsyncBatcher(fn, options = {}, selector = () => ({})) {
	...
	const [asyncBatcher] = (0, react.useState)(() => {
		...
		asyncBatcherInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useSelector)(asyncBatcherInstance.store, props.selector, { compare: _tanstack_react_store.shallow });
	...
	(0, react.useEffect)(() => {
		return () => {
			if (mergedOptions.onUnmount) mergedOptions.onUnmount(asyncBatcher);
			else {
				asyncBatcher.cancel();
				asyncBatcher.abort();
			}
	...
	const state = (0, _tanstack_react_store.useSelector)(asyncBatcher.store, selector, { compare: _tanstack_react_store.shallow });
