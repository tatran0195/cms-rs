...
var AsyncQueuer = class {
	...
	constructor(fn, initialOptions = {}) {
		...
		this.#tick = () => {
			...
			if (!scheduledAsyncWork) this.#setState({ pendingTick: false });
		};
		...
		this.execute = async (position) => {
			...
			if (item !== void 0) {
				...
				try {
					...
					if (activeItemIndex !== -1) remainingActiveItems.splice(activeItemIndex, 1);
					this.#setState({
		...
		this.flush = async (numberOfItems = this.store.state.items.length, position) => {
			this.#clearTimeouts();
			const results = await Promise.allSettled(Array.from({ length: numberOfItems }, () => this.execute(position)));
			this.#setState({ pendingTick: false });
		...
		this.flushAsBatch = async (batchFunction) => {
			...
			try {
				...
				this.#setState({ pendingTick: false });
				if (this.store.state.isRunning && this.store.state.items.length > 0) this.#tick();
			}
		};
		this.#checkExpiredItems = () => {
