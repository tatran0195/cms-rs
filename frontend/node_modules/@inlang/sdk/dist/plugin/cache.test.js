...
test("it should be network-first", async () => {
    ...
    const cachedPlugins = (await lix.execute("SELECT content FROM lix_file WHERE path LIKE '/cache/plugins/%'")).rows;
    expect(cachedPlugins.length).toBe(1);
    const parsed = new TextDecoder().decode(cachedPlugins[0].value("content").asBytes());
    ...
    const cachedPlugins2 = (await lix.execute("SELECT content FROM lix_file WHERE path LIKE '/cache/plugins/%'")).rows;
    expect(cachedPlugins2.length).toBe(1);
    const parsed2 = new TextDecoder().decode(cachedPlugins2[0].value("content").asBytes());
