...
test("serializes bundles with nested messages and variants", async () => {
    ...
    const nested = await selectBundleNested(reopened.db).executeTakeFirstOrThrow();
    ...
    expect((await selectBundleNested(reopenedPrevious.db).executeTakeFirstOrThrow())
        .messages[0]?.variants[0]?.id).toBe("welcome_en_admin");
    await reopenedPrevious.close();
    const legacyBlob = new Blob([
        JSON.stringify({
            format: "inlang-lix-memory-v1",
            files: previousFiles,
            bundles: serialized.bundles.map((bundle) => ({
                id: bundle.id,
                declarations: bundle.declarations,
            })),
            messages: serialized.bundles.flatMap((bundle) => bundle.messages.map((message) => ({
                id: message.id,
                bundleId: message.bundleId,
                locale: message.locale,
                selectors: message.selectors,
            }))),
            variants: serialized.bundles.flatMap((bundle) => bundle.messages.flatMap((message) => message.variants)),
        }),
    ]);
    const reopenedLegacy = await loadProjectInMemory({ blob: legacyBlob });
    expect(await reopenedLegacy.id.get()).toBe(serialized.lixId);
    expect((await selectBundleNested(reopenedLegacy.db).executeTakeFirstOrThrow())
        .messages[0]?.variants[0]?.id).toBe("welcome_en_admin");
