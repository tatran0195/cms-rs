...
export async function projectToBlob(lix) {
    const db = initDb({ lix });
    let lixIdResult;
    let files;
    let bundles;
    ...
    if (typeof lixId !== "string")
        throw new Error("Missing Lix id");
    const snapshot = {
        format: FORMAT,
        lixId,
        files: files.rows
...
export async function restoreProjectBlob(lix, blob) {
    let parsed;
    try {
        parsed = JSON.parse(await blob.text());
    }
    catch (cause) {
        throw new Error("The project uses the legacy Lix SQLite format, which Lix 0.9 cannot open in memory.", { cause });
    }
    ...
    const bundles = snapshot.format === LEGACY_FORMAT
...
function nestLegacyBundles(snapshot) {
    ...
    const messagesByBundle = new Map();
