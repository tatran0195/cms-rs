...
async function syncLixFsFiles(args) {
    ...
    async function checkLixState(currentLixState) {
        ...
        const filesInLix = (await args.lix.execute("SELECT path, content FROM lix_file WHERE path NOT LIKE $1", ["%db.sqlite"])).rows.map((row) => ({
            path: row.get("path"),
        ...
        for (const fileInLix of filesInLix) {
            ...
            if (!currentStateOfFileInLix) {
                ...
                if (arrayBuffersEqual(currentStateOfFileInLix.content, fileInLix.content.buffer)) {
...
async function upsertFileInLix(args, path, data) {
    ...
    await args.lix.execute("INSERT INTO lix_file (path, content) VALUES ($1, $2) ON CONFLICT (path) DO UPDATE SET content = excluded.content", [posixPath, new Uint8Array(data)]);
}
/**
