...
test("it should have the lix id as project id", async () => {
    ...
    const lixIdResult = await project.lix.execute("SELECT value FROM lix_key_value WHERE key = 'lix_id'");
    const lixId = lixIdResult.rows[0]?.value("value").toJS();
    const projectId = await project.id.get();
    expect(projectId).toBeDefined();
    expect(projectId).toBe(lixId);
    const legacyProjectId = await project.lix.execute("SELECT path FROM lix_file WHERE path = '/project_id'");
...
test("it should not persist the removed SDK metadata key during project creation", async () => {
    ...
    const removedKey = await project.lix.execute("SELECT value FROM lix_key_value WHERE key = $1", [removedSdkKey]);
