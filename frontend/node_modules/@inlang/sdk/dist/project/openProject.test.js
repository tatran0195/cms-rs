...
test("opens a project on a caller-owned Lix", async () => {
    ...
    const registeredSchemas = await lix.execute("SELECT value ->> 'key' AS schema_key FROM lix_registered_schema");
    expect(registeredSchemas.rows
        .map((row) => row.value("schema_key").toJS())
        .filter((key) => typeof key === "string" && key.startsWith("inlang_"))
        .sort()).toEqual(["inlang_bundle", "inlang_message", "inlang_variant"]);
    await expect(lix.execute("SELECT key, value FROM lix_key_value")).resolves.toBeDefined();
    await expect(lix.execute("SELECT id, name FROM lix_account")).resolves.toBeDefined();
    await project.db
        .insertInto("bundle")
        .values({ id: "caller-owned-lix" })
        .execute();
    expect(await project.db
        .selectFrom("bundle")
        .select("id")
        .where("id", "=", "caller-owned-lix")
        .executeTakeFirst()).toEqual({ id: "caller-owned-lix" });
    await project.close();
...
//# sourceMappingURL=openProject.test.js.map