...
async function selectLixFiles(lix, where = "", params = []) {
    const result = await lix.execute(`SELECT path, content FROM lix_file ${where}`, params);
    return result.rows.map((row) => ({
        path: row.get("path"),
        content: row.value("content").asBytes() ?? new Uint8Array(),
    }));
}
async function selectLixId(lix) {
    const result = await lix.execute("SELECT value FROM lix_key_value WHERE key = 'lix_id'");
    return result.rows[0]?.value("value").toJS();
}
async function selectLixFile(lix, path) {
...
describe("it should keep files between the inlang directory and lix in sync", async () => {
    ...
    test("file created in fs should be avaialable in lix ", async () => {
        ...
        const randomFileInLix = await selectLixFile(project.lix, "/file-created-on-fs.txt");
        expect(new TextDecoder().decode(randomFileInLix.content)).toBe("value written by fs");
    });
    test("file updated in fs should be avaialable in lix ", async () => {
        ...
        const settingsAfterUpdateOnDisk = JSON.parse(new TextDecoder().decode(fileInLix.content));
    ...
    test("file deleted in fs should be droped from lix ", async () => {
        ...
        const filesInLixBefore = await selectLixFiles(project.lix, "WHERE path = $1", ["/README.md"]);
        expect(filesInLixBefore.length).toBe(1);
        // "changes to a file on disk should reflect in lix
        fs.unlinkSync("/project.inlang/README.md");
        // console.log("wrting fs settings");
        await new Promise((resolve) => setTimeout(resolve, syncInterval + 10));
        const fileInLixAfter = await selectLixFiles(project.lix, "WHERE path = $1", ["/README.md"]);
    ...
    test("file created in lix should be avaialable in fs ", async () => {
        ...
        await project.lix.execute("INSERT INTO lix_file (path, content) VALUES ($1, $2)", ["/file-created-in.lix.txt", new TextEncoder().encode("random value lix")]);
        // lets wait a seconds to allow the sync process catch up
        await new Promise((resolve) => setTimeout(resolve, syncInterval + 10));
    ...
    test("file updated in lix should be avaialable in fs ", async () => {
        ...
        await project.lix.execute("UPDATE lix_file SET content = $1 WHERE path = $2", [
            new TextEncoder().encode(JSON.stringify({ ...mockSettings, baseLocale: "brand-new-locale2" })),
            "/settings.json",
        ]);
        // lets wait a seconds to allow the sync process catch up
        await new Promise((resolve) => setTimeout(resolve, syncInterval + 10));
    ...
    test("file updated in fs and lix (conflicting) should result in the fs state", async () => {
        ...
        await project.lix.execute("UPDATE lix_file SET content = $1 WHERE path = $2", [
            new TextEncoder().encode(JSON.stringify({ ...mockSettings, baseLocale: "lix-version" })),
            "/settings.json",
        ]);
        // lets wait a seconds to allow the sync process catch up
        ...
        const settingsAfterUpdateOnDiskAndLix = JSON.parse(new TextDecoder().decode(fileInLixUpdated.content));
...
test("it can import plugins via http", async () => {
    ...
    expect(pluginCache.some((file) => new TextDecoder().decode(file.content) === mockPluginModule), "expecting the plugin to be cached").toBe(true);
});
