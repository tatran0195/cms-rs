...
import { v7 } from "uuid";
const messageReferenceKey = (bundleId, locale) => JSON.stringify([bundleId, locale]);
const variantReferenceKey = (bundleId, locale, matches) => JSON.stringify([bundleId, locale, matches]);
 ...
 */
async function insertInBatchesByShape(args) {
    const rowsByShape = new Map();
    for (const row of args.rows) {
        const shape = args.optionalColumns
            .map((column) => row[column] === undefined ? "0" : "1")
            .join("");
    ...
    for (const rowsForShape of rowsByShape.values()) {
        for (let offset = 0; offset < rowsForShape.length; offset += INSERT_BATCH_SIZE) {
