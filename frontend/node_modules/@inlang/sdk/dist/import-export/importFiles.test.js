...
test("batch imports an unambiguous fresh project", async () => {
    ...
    expect(new Set(variants.map((variant) => variant.messageId))).toEqual(new Set(messages.map((message) => message.id)));
});
test("batches rows with mixed optional columns", async () => {
    ...
    const variants = await project.db.selectFrom("variant").selectAll().execute();
    expect(messages).toHaveLength(2);
    expect(messages.find((message) => message.locale === "en")?.selectors).toStrictEqual([]);
    expect(messages.find((message) => message.locale === "de")?.selectors).toStrictEqual([{ type: "variable-reference", name: "platform" }]);
    expect(variants).toHaveLength(2);
    expect(variants.find((variant) => variant.matches.length === 0)?.pattern).toStrictEqual([]);
    expect(variants.find((variant) => variant.matches.length > 0)?.pattern).toStrictEqual([{ type: "text", value: "Hello web" }]);
});
