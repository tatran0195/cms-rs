...
export const maybeAddSherlock = async (ctx) => {
    ...
    if (!isCertainlyVsCode) {
        isUsingVsCode = await prompt(`Are you using Visual Studio Code or a compatible fork (like Cursor)?`, {
            type: "confirm",
            initial: true,
        });
    }
    if (isUsingVsCode === false)
        return ctx;
    const shouldAddSherlock = await prompt(`Do you want to use the Sherlock VSCode (or Cursor) extension for a better DX?`, {
        type: "confirm",
        initial: true,
    });
    if (shouldAddSherlock === false)
        return ctx;
    ...
    if (settings.modules?.some((m) => m.includes("plugin-m-function-matcher")) ===
        ...
        settings.modules.push("https://cdn.jsdelivr.net/npm/@inlang/plugin-m-function-matcher@2.2.6/dist/index.js");
...
//# sourceMappingURL=maybe-add-sherlock.js.map